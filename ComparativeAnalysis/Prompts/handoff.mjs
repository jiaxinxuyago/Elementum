...
When the filled files come back: save them as \`ComparativeAnalysis/Prompts/out/handoff/${today}-${stemFile}-${chartId}-${pass}/<model>/<file>\` and run \`node validate-template.mjs <file>\` (the gate) and \`node render-template.mjs <dir> --against <benchmark dir>\` (the page-by-page read).
