# BLIND GENERATION TEST · STEM_BAND.yourNature_desc+self_card · geng_concentrated · concentrated
...
- Exemplar (庚 Overfueled): "[withheld: this exemplar is the cell under test]" · (庚 Underfueled): "You keep that edge, under cloth and out of view. Seeing through things is instant for you. Saying so out loud is the part that comes and goes…"
...
- Construct: `face` names the band state in the element's arena ("[withheld: this exemplar is the cell under test]"); `presence` is being that state, dignified even on the Underfueled band.
...
- Exemplar (庚 Overfueled): face "[withheld: this exemplar is the cell under test]" · presence "[withheld: this exemplar is the cell under test]"
...
Angle not to borrow: not the unwitnessed-martyr angle unless this stem is 丙, 己 or 庚.
Band: concentrated = Overfueled (the core has more than it can channel: the edge never sheathed).
- manifesto: "Precision before intention · You are the Metal that cuts things clean."
- dm_overview (the sign whose central image sentence 1 must receive): "The Blade is Yang Metal, the metal that has already been through fire. It arrives with autumn, the season that ends what summer started, and it does the harvest's honest arithmetic, deciding what was worth growing. Kept plain and kept sharp, it is drawn once, only for the true thing. Between wars it hangs on the wall and says nothing, which is its own kind of speech."
- yourNature_desc (baseline, band-neutral, for the person's trait): "You put the truth ahead of your own comfort, and usually ahead of your own company. People keep the clarity you hand them and stay careful around the edge it came from. The cost never comes up. You've never once shown anyone the bill."
...
Write {"yourNature_desc": "...", "self_card": {"face": "...", "presence": "..."}} for 庚 The Blade on the Overfueled band: yourNature_desc 50–75 words opening on "You" and receiving the sign's image in sentence 1; face ≤8 words; presence ≤30 words.

Output shape: {"yourNature_desc": "...", "self_card": {"face": "...", "presence": "..."}}
