# BLIND GENERATION TEST · ELEMENT_PAIR.mechanism.turn+carry · 火_木 · catalyst
...
Cell 火_木. Core element Fire, energy element Wood. Law: Wood feeds Fire (木生火: dry branches make the flame). Relative to the core the energy is the feeder, so Wood is this core's Mind.
- mechanism.classic (the sourced epigraph): "木性溫暖，火伏其中"
...
- carry.catalyst: {"clause":"The flame eats scraps: brilliance flaring on no reserves, warmth borrowed from tomorrow.","remedy":"Learn, gather, befriend, store."}
- carry.friction: {"clause":"The fire outgrows its hearth: every branch thrown on at once, heat spent in a rush.","remedy":"Burn one pile at a time."}
...
- carry.missing: {"clause":"No fuel behind the flame: brilliance on nothing, warmth borrowed from tomorrow.","remedy":"Borrow wood: learn, gather, store."}
- carry.spared: {"clause":"Wood runs absent, and the fire keeps its size: one flame, one task, burned to the end.","remedy":"Keep it. Gather only what you will burn."}
- carry.thin: {"clause":"Wood runs thin and unwanted: a small pile, easily burned.","remedy":"Burn it as it comes."}
...
The classical excess idiom for this cell (say it in English in carry.excess, never quote it): 火赖木生，木多火炽 (生之太过, the Mind door; read literally since D1: the fire blazes past its task).
...
Yin sibling noun set for a _yin target: the close flame, the table, the lamp (the Candle for the Sun's shared line).
...
The target is the YIN SIBLING'S variant (丁 The Candle): the shared mechanism and carry lines above name the yang archetype's noun; write the same story and the same directive with the yin sibling's noun (the close flame, the table, the lamp (the Candle for the Sun's shared line)). The yin line's echo of the shared line is a lawful match.

Chart under test: contrast 丁 The Candle, weak, one Order door (fixture: Wood 30 / Fire 15 / Earth 5 / Metal 10 / Water 40). Band Underfueled. Wood runs 30%: catalyst, abundant; the state under test is catalyst.

## TASK

Write for 火_木, the yin sibling's variant: (a) mechanism_yin.catalyst_turn (≤35 words, opens "Run thin,"); (b) carry_yin.catalyst = {clause ≤18w, remedy ≤12w} CUT from that turn; (c) carry_yin.missing = {clause, remedy} for Wood wanted and absent; (d) carry_yin.wide = {clause, remedy} for Wood wanted and abundant or dominant, the plentiful line opening with one of the eight wide openers. Return {"catalyst_turn": "...", "carry": {"catalyst": {...}, "missing": {...}, "wide": {...}}} only.
