# BLIND GENERATION TEST · ELEMENT_PAIR.mechanism.turn+carry · 金_土 · friction
...
**`ELEMENT_PAIR.mechanism.catalyst_turn`** · **`ELEMENT_PAIR.mechanism.catalyst_turn` / `friction_turn`** · energy page, the state line (also the carry card's source) · classical, personal · second person allowed · ≤35 words each · LOCKED
...
- Exemplar (金_土 friction_turn for 辛): "[withheld: this exemplar is the cell under test]"
...
- Construct: the yin sibling's noun in the carry lines, same mechanism, same directive. Exemplar (金_土 excess for 辛): "[withheld: this exemplar is the cell under test]" / "[withheld: this exemplar is the cell under test]" Sources: REA_02 §5h (2026-09-16).

---

## CELL FACTS

Cell 金_土. Core element Metal, energy element Earth. Law: Earth feeds Metal (土生金: ore grows in the mountain). Relative to the core the energy is the feeder, so Earth is this core's Mind.
- mechanism.classic (the sourced epigraph): "金居石依山，津潤而生"
...
- mechanism_yin.friction_turn: [REDACTED: the target field]
...
- carry.catalyst: {"clause":"The mountain goes hungry: a keen edge with nothing behind it, sharpness spent faster than it forms.","remedy":"Rest and study are smelting, not stalling."}
- carry.friction: {"clause":"The shelter closes over the blade: so much preparation that the edge never leaves the mine.","remedy":"Dig out and cut something real."}
...
- carry.missing: {"clause":"No ground under the edge: judgment made on the spot, nothing settled in first.","remedy":"Borrow ground: a mentor, a book, one slow evening."}
- carry.spared: {"clause":"Earth runs absent, and the edge is lighter for it: nothing to bury it, nothing to shelter in.","remedy":"Keep it that way. Decide, then rest."}
- carry.thin: {"clause":"Earth runs thin and unwanted: a little shelter, easily left.","remedy":"Leave it as it is, small."}
...
- carry_yin.catalyst: {"clause":"The mine goes hungry: a fine stone with nothing behind it, brilliance spent faster than it forms.","remedy":"Rest and study are the polishing, not stalling."}
- carry_yin.friction: [REDACTED: the target field]
- carry_yin.excess: [REDACTED: the target field]

The classical excess idiom for this cell (say it in English in carry.excess, never quote it): 金赖土生，土多金埋 (生之太过, the Mind door; 土多金埋 applies to 辛, 庚 reads 郁滞).
The eight wide openers (a carry.wide clause opens with one; no core repeats one across its wanted energies): "{Element} is one thing you never run short of" · "{Element} is easy for you to reach" · "{Element} is all around you" · "You have plenty of {Element}" · "{Element} is already here in plenty" · "You are well supplied with {Element}" · "You were given plenty of {Element}" · "There is no shortage of {Element} in you".
Yin sibling noun set for a _yin target: stone, facet, setting, wheel (the Jewel for the Blade's shared line).
...
The target is the YIN SIBLING'S variant (辛 The Jewel): the shared mechanism and carry lines above name the yang archetype's noun; write the same story and the same directive with the yin sibling's noun (stone, facet, setting, wheel (the Jewel for the Blade's shared line)). The yin line's echo of the shared line is a lawful match.

Chart under test: contrast 辛 The Jewel, Overfueled, heavy Earth (fixture: Metal 20 / Earth 45 / Wood 15 / Fire 10 / Water 10). Band Overfueled. Earth runs 45%: friction, dominant, the excess override applies; the state under test is friction.

## TASK

Write for 金_土, the yin sibling's variant: (a) mechanism_yin.friction_turn (≤35 words, opens "Run heavy,"); (b) carry_yin.friction = {clause ≤18w, remedy ≤12w} CUT from that turn; (c) carry_yin.thin = {clause, remedy} for Earth unwanted at ≤10%; (d) carry_yin.excess = {clause, remedy} for dominant Earth, the cell's idiom in English, never quoted. Return {"friction_turn": "...", "carry": {"friction": {...}, "thin": {...}, "excess": {...}}} only.

Output shape: {"friction_turn": "...", "carry": {"friction": {"clause","remedy"}, "thin": {...}, "excess": {...}}}
