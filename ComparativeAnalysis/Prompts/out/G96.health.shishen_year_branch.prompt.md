# BLIND GENERATION TEST · POSITION.health_line · shishen_year_branch
...
Named event: "The Artisan inside the Year Gate" · 食神在年支. Persona The Artisan (食神). Definition line: "Output that flows from you, giving that feels like being". Keyword Flow, charge gentle, poles catalyst Grace / friction Coasting. Family function: Expression. Cost dimension: giving that feels like being; worth tangled with what is made. Arena: craft, hands, the table, warmth of making, fed and feeding. Classical portrait: ease and appetite; output that flows without assertion; generous, content, prone to drift and indulgence. Ruled domains: Expression, Enjoyment, Children. Sibling (other polarity of the family): 伤官.
Gate: Year Gate (ground: roots, ancestry, the early world). Slot kind: branch (branch = the root, private, dispositional, "inside the Gate"). Era: the Early Chapter (~0–16). Relations register: ancestors and the family's name. Seat rank: sixth of seven (年支). Declared domains: ["Family","Health"].
...
Write health_line for shishen_year_branch as {"health_line": "..."}: ≤30 words, wellness register, non-medical: the seat's body correspondence in gentle tendency language; never a symptom assigned as fact, never a diagnosis.
