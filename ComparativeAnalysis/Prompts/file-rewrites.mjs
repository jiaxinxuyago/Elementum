...
for (const [key, c] of Object.entries(byAxis)) { const [axis, cell] = key.split('/'); const d = `${REW}by_axis/json/${axis}/`; fs.mkdirSync(d, { recursive: true }); fs.writeFileSync(`${d}${cell}.${model}.json`, JSON.stringify(c, null, 2)); const md = `${REW}by_axis/md/${axis}/`; fs.mkdirSync(md, { recursive: true }); fs.writeFileSync(`${md}${cell}.${model}.md`, `# ${axis} · ${cell} · generated by ${model} (${today})\n\n> A REWRITE STATION twin. Truth is Reading/Database/templates/by_axis/json. Filed from ${handoffTag}.\n\n| Field | Original | ${model} | Gate |\n|---|---|---|---|\n` + Object.entries(c.$gate_findings).map(([fp, g]) => { const flat = (o) => String(typeof o === 'string' ? o : JSON.stringify(o ?? null)).replace(/\|/g, '\\|').replace(/\n/g, ' ').slice(0, 900); return `| ${fp} | ${flat(shown[key][fp].original)} | ${flat(shown[key][fp].candidate)} | ${g === 'pass' ? 'pass' : 'FAIL: ' + g.join('; ').replace(/\|/g, '\\|')} |`; }).join('\n') + '\n'); }
