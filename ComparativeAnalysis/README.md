...
- **`Handoffs/`** — self-contained packages for a model with zero background (built by `Prompts/handoff.mjs`): one dispatch prompt, the prompt database, the cards in scope, the chart, the skeletons to fill; zipped beside the folder. First: `2026-09-23-geng-golden-blind` and `-benchmarked` (the Blade's Day Master page and five energy pages on the golden chart, 54 fields).
