...
- **O11 · CLOSED (2026-09-21, second cloud session).** Primary texts read from git mirrors (the Kanseki 四庫全書 三命通會, a raw 滴天髓闡微 and 窮通寶鑑) after every web route stayed blocked. Burial: the reviewer's line exists, 三命通會 卷十二 元理賦 *土重金埋者陽金* (gloss: *在庚金則畏，辛金則否*), while 卷二 論天干陰陽生死 buries both stems and 滴天髓 keeps 埋 to 辛; so the REA_04 caveat is now recorded as a house choice following 滴天髓, in one sentence, wording and the `金_土` turn untouched (D5). 辛 with Fire: 滴天髓 *熱則喜母，寒則喜丁* and 窮通寶鑑 論辛金 (Fire only in the cold months, and only next to 壬水) make the welcome conditional; recorded as a `sources` row on `金_火.json` so `carry_yin.wide` reads as an interpretive analogy, station line unchanged.
- **O12 · LAPTOP.** The humanized-prose skill file is gitignored and absent from the cloud clone (confirmed again 2026-09-21 in a second cloud session: no copy anywhere on the container). Still the laptop task card: remove its trailing dash allowance, make sure no exemplar carries an em-dash, and log the fix in REA_16 §6.
