...
You are running inside the Elementum repository. The handoff package is at `ComparativeAnalysis/Handoffs/2026-09-23-geng-golden-benchmarked/` (no archive to unzip): read its `00_DISPATCH_PROMPT.md` and follow it; the `prompts/`, `cards/`, `chart/` and `skeletons/` folders beside it are the files it names.
...
Only the six filled skeletons, to `ComparativeAnalysis/Prompts/out/handoff/2026-09-23-geng-golden-benchmarked/<your model name>/<same file names>.json`, each with `"_generated_by"` at the top. Nothing else: not `Reading/Database/templates/`, not `Elementum_App/src/content/`, not the prompt database, not the handoff folder itself. Commit on a branch named `handoff/2026-09-23-geng-golden-benchmarked/<your model name>`, never on `main`.
...
- `node ComparativeAnalysis/Prompts/render-template.mjs <your output folder> --against ComparativeAnalysis/Handoffs/2026-09-23-geng-golden-benchmarked/benchmark`: your pages beside the current text, for your own read.
