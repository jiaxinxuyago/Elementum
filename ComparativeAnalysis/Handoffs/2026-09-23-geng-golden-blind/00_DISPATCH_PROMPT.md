# DISPATCH · Elementum reading, 庚 The Blade, chart golden · blind pass
...
Each skeleton is one page of the reading: the Day Master page, then one energy page per energy. Each carries the page's facts (the cell's other fields, the chemistry, the chart's state for that energy; the fields under test are redacted), and one entry per field with a `spec` (the card it follows, the word range, the person, the opener where one is required, the notes) and an empty `value`.
