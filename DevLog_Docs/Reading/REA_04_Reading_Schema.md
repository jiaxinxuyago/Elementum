...
> **Formerly DES_05** (moved to the Reading library in the 2026-07-23 design/reading doc separation) **and before that DOC6** (2026-07-09 reorganization). Historical citations of "DES_05" or "DOC6" refer to this file (registry: Documents/README.md).
...
Why 50 is exact: per DM element, each of the 5 elements resolves to one god *pair* (2 polarity registers) → 10 pairs per DM element × 5 DM elements = 50 valid (element, god) combinations. This is REA_01's Layer-2 taxonomy, unchanged — **the D13 surface finally gives Layer 2 its native home.**
...
> **Implementation chain.** This field spec is the **design source of truth** for the reading content model. `archetypeSchema.js` (code) implements it — types, copy caps, tiers, `varyBy` cardinality — and **REA_06** mirrors that for designers, grouped by UI surface. Order: freeze this spec → rewrite `archetypeSchema.js` to it (code pass) → regenerate REA_06. Until then, `archetypeSchema.js` + REA_06 still describe the legacy v0.x model.
...
### Decided — v2.1 (2026-06-24, owner-locked · record: `REA_08_Reading_V2.1_Reconciliation_Audit.md`)
...
- **B6** **Position (宫位) is a first-class reading axis** (alongside the FACES dominance×polarity axis). Each Ten God reads by the pillar it occupies; a positional reading = **canon Ten-God mechanism × palace life-domain × polarity** (REA_02 §2.7b). Composes from K2 + ~7 new `PALACE_FRAMES`; per-pillar TG data already in `chart.tenGods`. **日支 (spouse/夫妻宫)** and **时柱 (children/legacy)** are the highest-value reads. Elevates 宫位 from VERIFY-ONLY (REA_02) to a behavioral driver — methodology now blends 子平 (格局) + 宫位论法.

### Ruled — Cece-derived backlog housekeeping (owner, 2026-07-23)
The four pending Cece-derived proposals (D5–D8, from `Cece_BaZi_Reading_Analysis.md`) were re-reviewed against the current reading journey and its design goal — *an easier, clearer reading journey with minimal cognitive load and clear BaZi system knowledge for first-time learners*. Cece remains a benchmark for **reading angles and content only**, not design or product strategy.
- **D5 — rarity marker on the identity plate: RETIRED.** A virality mechanic, not a reading angle; adds a classification stat with no BaZi teaching value. The plate stays pure identity.
- **D6 — 大运 decade timeline: REFIT → backlog.** The Cece density framing and its implementation premise (reuse `CYCLE_LINES` — since removed from src) are dropped. Refit as: *the reading journey should teach the 大运/cycle concept — surface and form TBD during reading-journey content design.* Revisit after content design.
- **D7 — FREE-tier gating: STAYS · principle locked.** Standing direction: **gate by depth, never by hiding cards or truncating text** — the free reading must feel complete, paid feels deeper. The concrete free/paid split waits until content design defines the depth blocks (interacts with open #3 below).
- **D8 — cohort badge / social proof: RETIRED.** Pure product-strategy virality resolving jointly with D5; falls with it. No social-proof surface in the reading.
