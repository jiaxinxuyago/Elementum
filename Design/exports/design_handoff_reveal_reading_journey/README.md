...
- **⚠ Owner-decision flag:** in the current build the **seal dock is hidden** (`.ca-dock{display:none}`) — the footer chips take the scroll end and the pills are the sole element index. The dock still exists in p6-journey + the boards for rollback. Confirm retire-vs-keep before wiring; implement per share-flow (no dock) unless overruled.
