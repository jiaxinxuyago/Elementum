...
const s7 = col(
  {v:'Reading tab · drill-down', n:'Reading detail', d:'Sticky chapter header + progress; the one scrolling reading. Scroll to pin the bar. Pager is its nav.'},
  statusbar() + `<div class="scr">
    <div class="scrollwrap" data-init-top="0">
      <div class="hero" style="${pv('metal')}height:298px;border-radius:0;border:none;box-shadow:none">
...
const s8 = col(
  {v:'Profile tab · Energy Map', n:'Energy Map', d:'Ceremonial “The Blade” reveal fills one screen; scroll reveals the composition wheel + Enter.'},
  statusbar() + `<div class="scr">
    ${pagebg('earth',{img:'earth-wu',imgOpacity:0.16,tint:'linear-gradient(180deg,rgba(231,223,201,.2),transparent 30%,rgba(215,211,200,.3) 70%,transparent)'})}
    <div class="scrollwrap" data-init-top="0">
      ...
      <div class="reveal">
        ...
        <div style="width:120px;height:1px;background:var(--inkMist);opacity:.6;margin:16px 0 14px"></div>
        <div class="serif italic" style="font-size:16px;color:var(--inkSoft)">Precision before intention</div>
        <div class="chips3">
          <div class="chip3">${mark('metal')}<span class="eyebrow">Metal</span></div>
          <div class="chip3"><span class="hz" style="font-size:27px">庚</span><span class="eyebrow">Geng</span></div>
          <div class="chip3"><span style="width:22px;height:22px;border-radius:999px;border:1.5px solid var(--inkLight);background:linear-gradient(90deg,var(--inkLight) 50%,transparent 50%)"></span><span class="eyebrow">Yang</span></div>
        </div>
      ...
      <div style="padding:0 16px 26px">
        <div class="cardstock blueprint" style="${pv('metal')}padding:15px 16px">
          ...
          <div style="flex:1"><div class="serif" style="font-size:21px;font-weight:600">Metal <span class="pill pig" style="--pigDeep:var(--metalDeep);--pigBd:var(--metal);margin-left:4px">Concentrated</span></div>
...
/* ---------- 9 · GUIDANCE HUB ---------- */
function gtile(o){
  return `<div class="cardstock" style="${pv(o.el)}padding:13px;display:flex;flex-direction:column;justify-content:space-between;gap:8px;border-left:3px solid var(--pig)">
    <div class="row" style="justify-content:space-between;align-items:flex-start">
      <span style="width:34px;height:34px;border-radius:9px;background:var(--pigSoft);border:1px solid var(--pigBd);display:flex;align-items:center;justify-content:center;color:var(--pigDeep)">${o.locked?ico('ico-lock'):mark(o.el)}</span>
...
const s9 = col(
  {v:'Guidance tab', n:'Guidance hub', d:'Daily Draw featured; four deeper features as a 2×2 grid. Gated cards carry a lock + tier.'},
  statusbar() + `<div class="scr">
    ${pagebg('wood',{tint:'radial-gradient(120% 70% at 90% 8%, rgba(122,94,154,.07), transparent 55%)'})}
    <div class="pad">
      ...
      <div class="cardstock" style="${pv('wood')}padding:15px 16px;display:flex;flex-direction:column;gap:11px;border-left:3px solid var(--pig)">
        <span style="width:44px;height:44px;border-radius:11px;background:var(--pigSoft);border:1px solid var(--pigBd);display:flex;align-items:center;justify-content:center;color:var(--pigDeep)">${mark('wood')}</span>
        <div><div class="serif" style="font-size:20px;font-weight:600">Elemental Draw</div>
          <p class="body" style="margin-top:6px;font-size:13px">A daily question drawn from the day’s elemental current — today’s deck follows your Today tab.</p></div>
        <div><button class="btn btn-outline-pill" style="${pv('wood')}border-color:var(--pigBd);color:var(--pigDeep)">Draw today’s card <span class="arr">→</span></button></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:11px;margin-top:2px">
        ${gtile({el:'earth',locked:true,tier:'Seeker',title:'Energy<br>Manual',desc:'Five life domains.'})}
        ${gtile({el:'water',locked:true,tier:'Seeker',title:'Self-<br>Report',desc:'Tune your readings.'})}
        ${gtile({el:'water',locked:true,tier:'Advisor',title:'AI<br>Consultant',desc:'Ask anything.'})}
        ${gtile({el:'fire',title:'BaZi<br>Codex',desc:'The concepts, explained.'})}
      </div>
    </div>
    ${tabbar('guidance')}
  </div>`
...
function cpcard(o){
  return `<div class="cpcard" style="${pv(o.el)}"><div class="cpart">${art(o.art,o.el)}</div>
    <span class="eyebrow" style="position:relative">${o.who}</span>
    <span class="cpmark">${mark(o.el)}</span>
    <div style="position:relative"><div class="hz" style="font-size:24px;color:var(--ink)">${o.hz}</div>
...
const s10 = col(
  {v:'Compat tab · result', n:'Compatibility result', d:'Versus split with the score medallion bridging the two portraits at the seam.'},
  statusbar() + `<div class="scr">
    ${pagebg('metal',{tint:'linear-gradient(180deg,rgba(231,223,201,.25),transparent 34%,rgba(215,211,200,.3))'})}
    <div class="pad tight" style="padding-top:6px">
      ...
      <div class="cpair">
        ${cpcard({el:'metal',who:'You',hz:'庚',name:'The Blade',pol:'Yang Metal',art:'metal-geng'})}
        ${cpcard({el:'metal',who:'Mara',hz:'辛',name:'The Jewel',pol:'Yin Metal',art:'metal-xin'})}
...
const s11 = col(
  {v:'Profile tab', n:'Profile (Me)', d:'Birth-data grid, notifications + account, sign out. Quietest screen in the system.'},
  statusbar() + `<div class="scr">
    ${pagebg('metal',{tint:'radial-gradient(120% 50% at 50% 110%, rgba(139,163,184,.08), transparent 60%)'})}
    <div class="pad">
...
const s12 = col(
  {v:'Profile → drill-down', n:'Raw Chart + Resonance', d:'Four pillars (Day = Self, Hour unconfirmed), legend, and the Resonance entry. No tab bar.'},
  statusbar() + `<div class="scr">
    ${pagebg('metal',{img:'enso',imgOpacity:0.05})}
    <div class="pad tight">
...
window.SCREENS_B = [s7,s8,s9,s10,s11,s12];
})();
