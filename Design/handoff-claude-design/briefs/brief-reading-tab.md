...
> **⚠ v2.1 RECONCILIATION (2026-06-24 · see `DevLog_Docs/Reading/REA_08_Reading_V2.1_Reconciliation_Audit.md`).** This brief now adds a **FACES prologue** between the element (catalogue) and its text reading: tapping an element opens **1–2 persona character-cards by calculation** (dominant-led; second face only if its polarity is genuinely present — no manufactured latent cards), each carrying the dominant-energy abstract + punchline + keywords + **ruling-domain** line. The reading is **persona-scoped** and reads at a depth set by its **presence frame**. Persona art = the 10 Inner-Council characters recolored per element. Add acceptance criteria for the Faces prologue + ruling-domain line. **Yin/Yang correction:** keep the DM *identity* Yang/Yin chip (permitted stem register); face cards carry a **subtle Yin/Yang marker** (B7). The Faces prologue is fully specced as the **Energy Faces screen** (DES_04 §11). **Positional axis (宫位, B6)** — a *separate* per-pillar surface — is out of scope for this catalogue brief but cross-linked.
