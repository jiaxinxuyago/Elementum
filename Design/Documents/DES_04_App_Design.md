...
**Reading order:** Read REA_01 (the compound-system overview), then REA_02 §2 (names) and REA_03 §3–§4 (the data variables) before this document. DES_04 describes how to render the data; the Reading library describes what the data is.
...
- The **reading-structure redesign** (D12): section composition, per-section jobs, field formats, and word budgets (evidence base: `the retired format audit (git)` — budgets serve the voice, not vice versa).
...
**Related:** §1 (the ancient-seriousness ↔ daily-accessibility tension this charter operationalizes) · REA_05 (voice + generation rules) · `DES_13_Design_Audit_Backlog.md` D12 · `the retired format audit (git)` (format evidence).
...
**Tap a face card → the persona's reading.** The prologue content (abstract / punchline / chips / ruling domain) already lives on the card; the destination is the text-heavy read — `R · X · gate · seeker` in **presence-frame registers** (REA_03 §3–§4).
...
**Relationship to the positional axis (B6):** this screen is the **element-dominance** view. The **positional reading (宫位 × 十神)** is a *separate* per-pillar surface (年/月/日支-夫妻宫/时) reusing the same persona content framed by palace — not part of this screen. See REA_03 §3 + REA_04 §2.7b.
...
> 6. **Monetization status — WIRED (same day):** real $6.99 Payment Link (account-gated; client_reference_id attached; Google-OAuth purchase-intent resume), webhook grants has_self_report server-side (amount-routed), ?selfreport=ok return polls then lands the buyer in app-selfreport. The localStorage demo flip no longer grants — server-truth like the tier.
...
2. **Dot card · the minimal hook** *(amended by owner rulings 2026-09-01, final form after the cleanup pass)*: rung ② is a HOOK, not a summary — the card carries exactly four layers plus the entry: (a) band identity (the element's ink wash + name + 汉字, replacing the old identity row), (b) the 生/克 mechanism graphic (presence-filled capsules + law glyph) with the **role pill centered beneath it** as the card's only state signal (the equation caption retired — the graphic already explains the relation), (c) the DEFINITION + VERDICT pair (owner formula): a clean display definition speaking the §5f function claim ("Wood is your **Action**") over one verdict sentence in the position-teaser register explaining what the definition means (`ELEMENT_PAIR.cta_verdict` ×25 LOCKED, ≤30w, behavioral truth + one tendency-framed consequence beat — "Pressure is your forge, and the heat you choose on purpose finishes an edge that comfort would have left dull."), (d) three role-conditioned keyword chips, then (e) the **arrow-only reading entry** (the readcirc language: 38px dark circle + arrow, centered — consistent with every other reading-page entry; the old tile's title and seat-destination subtext retired). **Retired from the card across the passes:** the family line, the state verdict sentence, the presence track, the separate identity row, the cycle image line, the seat body line, the equation caption, and the CTA tile text — each covered downstream (the deep seat stays on the catalogue takeaway strip and the element page).
3. **Two-stage seat disclosure** (Seeker): a seat opens to Stage A = summary reading + domain paragraphs (with woven pattern + tone turn); "The deeper layers" expander reveals Stage B = life chapter, the people, the shadow, the body. **Free-tier Stage A teaser (softener, owner 2026-08-19):** a free seat shows the summary reading's declaration sentence plus a seat-specific inventory lock line ("With Seeker this seat opens in full: the reading, Mind, Growth, Career, your rising chapter, the people, the shadow, and the body.") — derived, no new content, D7 depth-hunger form.
4. **Seat ordering + arrival state** *(amended 2026-09-01)*: the Domains detail and teaser rows list seats by the significance ranking (below); the top seat's Stage A **auto-opens on arrival** — the reading is visible with zero taps (this replaced the START HERE mark, which was retired: the ranking leads by showing, not labeling).
5. **The seat panel's revelation ladder** *(owner 2026-09-01: "re-arrange the sequence of the revelation… without cognitive load")*: inside an open seat the READING LEADS — summary reading → domain paragraphs (pattern woven) → tone turn → the deeper layers → **the god bridge** ("About the Horizon, who holds this seat →", which opens and scrolls to that god's block). The domain chips + defline demoted to the free tier only (Seeker's reading declares its domains in its own first sentence). The WHO RUNS IT god layer is **collapsed accordions** (head = persona + 汉字 + "runs your {Function}" + share; body = defline, function line, inherent-domain chips, Seeker god-domain readings) — supplementary god teaching is opt-in and always introduced FROM the position, never against it.
...
| §AM.11 rung ② amended | 2026-09-01 | The minimal dot card (owner rulings ×4): the energy CTA slimmed from nine information layers to four — band identity, the 生/克 mechanism graphic, one derivation claim (role pill inline), three keyword chips — with the deep-seat destination riding as the CTA subtext. Family line, state verdict, presence track, identity row, cycle image line, and seat body line retired from the card (all covered downstream on the element page). |
| §AM.11 rung ② verdict line extended | 2026-09-01 | The dot card's meaning line now surfaces the seat's §5f function noun ("That is why Fire is your Duty. It runs your Order.") — the CTA verdict introduces each energy's function under the re-ruled bijection (Core→Body · Root→Mind · Voice→Expression · Drive→Action · Duty→Order). |
| §AM.11 rung ② definition + verdict | 2026-09-01 | The owner's verdict formula (from the deep-seat teaser exemplar): the card's text becomes a display DEFINITION ("Wood is your Action", function claim, pill inline) over a VERDICT sentence in the position-teaser register (`ELEMENT_PAIR.cta_verdict` ×25, ≤30w, truth + one consequence beat) — replacing the derivation sentence; the seat noun now lives on the element page only. *(Superseded 2026-09-02: the function-noun law went total — seat nouns retired from ALL surfaces and corpus; see REA_02 §5b.)* |
| §AM.11 header + mechanism card + black arrows | 2026-09-02 | The page header (thumbnail + diagnosis) is a pure header; THE MECHANISM gets its own section card; the mechanism detail opens UNDER the shared thumbnail; black arrow buttons on all reading tap-ins. *(Superseded same day: the standalone MECHANISM card duplicated the claim.)* |
| §AM.11 claim-clause strip | 2026-09-02 | The FUNCTION card's title and its reading opener both spoke the claim (within-card echo). Display de-dup, corpus untouched: since the claim titles the section, the reading renders with its claim clause stripped — "As a friction, it is intake…" / "Running over, it is…" — on both the card teaser and the detail ¶1 (derived in journeyData; the locked corpus keeps its full opener for any surface without a claim title). |
| §AM.11 mechanism reads from the diagnosis | 2026-09-02 | Owner caught two sections repeating "Earth is your Mind" — the standalone MECHANISM card retired; the reading button lives ON the diagnosis line inside the header card (thumbnail + diagnosis + black arrow), and the mechanism reading expands from there (detail unchanged: thumbnail header + base + turn). Page = header(→mechanism) · THE FUNCTION · THE DOMAINS — each claim spoken once. |
| §AM.11 THE COVER (shared thumbnail) | 2026-09-02 | The CTA journey re-ruled (owner forks ×3): the wheel-dot card IS the element page's one-thumbnail — the floating COVER carries the thumbnail + the rung-② texts (claim · cta_verdict · keyword chips · arrow), and tapping through lands on the page HEADED BY THE IDENTICAL THUMBNAIL (diagnosis footer = the first new text). One object across the transition: the chemistry equation is taught exactly once, then recognized. The dot card's bespoke band art + relation strip retired; the thumbnail/equation builders are parameterized and shared (heroThumbFor/mechVizFor). |
| §AM.11 one-thumbnail hero | 2026-09-02 | Section 1 redesigned as ONE coherent dark thumbnail: the ink art is the ground, and everything visual composes on it — role chip + identity eyebrow (top row) → the chemistry equation as the focal object (light capsules on dark wash, law text and captions in cream with hero shadows, state tag, "= role" conclusion) → dominance bar + % at the base; the diagnosis is the card's cream caption footer. The claim line ("Earth is your Mind.") moved off the art to become THE FUNCTION card's serif title. |
| §AM.11 element page re-org | 2026-09-02 | Owner's three-section order: ① thumbnail (condition in graphical language) ② equation + diagnosis → arrow into the MECHANISM detail (base + turn) ③ THE FUNCTION card (derived teaser = the reading's first sentence) → arrow into the FUNCTION reading (v3, 3 ¶: definition · day-to-day · the program, + dips coda). The card-per-destination language restored; definitions expanded 40–70w → 100–140w (template ×5 pending lock → batch ×20). |
| §AM.11 diagnosis equation | 2026-09-01 | The hero card after the thumbnail = the chemistry EQUATION graphic (core capsule state-tagged, caption concluding in the role: "EARTH FEEDS METAL = ↓ FRICTION") + ONE templated diagnosis verdict speaking the chain (state → chemistry → conclusion → directive). cta_verdict and the state-turn teaser retired from the hero (covered on the dot card / detail). Follow-up same day: the §5f FUNCTION paragraph returned, rendering under the thumbnail to answer the title's claim — hero = thumbnail → function ¶ → equation → diagnosis. |
| §AM.11 element hero card | 2026-09-01 | The element page's art band + mechanism card consolidated into one hero card (the energy-pill expanded card reborn): art thumbnail carrying role chip + name + the function claim as headline + the five-energy dominance track with the % beside it ON the art (owner refinements: number rides with the bar, rank caption removed, bar moved onto the thumbnail — light-on-dark) → capsule formula → cta_verdict → seek/skip diagnosis; taps into the mechanism detail. Tile hook + tag retired (ENERGY_TILE ore). Page = hero + Domains. |
| §AM.11 Domains revelation ladder | 2026-09-01 | Owner feedback: the disclosure was inverted (position readings folded, god paragraphs expanded). Fixed: top seat auto-opens (START HERE mark retired), the reading leads the seat panel (chips + defline demoted to free tier), WHO RUNS IT collapses to god accordions, and each seat ends with the god bridge ("About {persona}, who holds this seat"). Surfaces #4–#5 amended. |
| §AM.11 rung ② cleanup (verdicts locked) | 2026-09-01 | cta_verdict ×25 LOCKED. Design cleanup ×2 (owner): the equation caption retired (the graphic explains the relation) with the role pill taking its slot, leaving the definition line clean; the CTA tile collapsed to the arrow-only readcirc entry (title + seat-destination subtext retired — the deep seat stays on the catalogue strip and the element page). |
| §AM.11 function detail = the god-grain reading | 2026-09-03 | The function reading page re-anatomized (owner rulings: ELEMENT×GOD grain + option 2 for two-faced elements): title claim → define ¶ (claim-stripped) → the keyword LEDGER (the card's three chips, each a bold word + defining line + real-life scene — replacing both the bare-chip row and the one-line glosses) → advise ¶ → the derived FACE-SPLIT line when the element carries both polarity faces ("Your Wood runs two temperaments: mostly The Steward (正财 74%)… Meet The Horizon under The Domains", hairline-separated `.el-facesplit`) → dips coda. Unbatched god cells fall back to the pair-grain 3-¶ definition + bare chips. Card teaser now derives from the god-grain define. Verified on the exemplar's Wood (two-faced catalyst), Earth (single-faced friction), Metal (core-excess, two-faced) and a 甲 fallback cell. |
| §AM.11 the weighted ledger | 2026-09-03 | Owner rulings ×4 (direction 1 over god-blind element analysis, argued from 子平 doctrine — 以强者论 + 混杂 reading): the ledger becomes the element's full polarity composition, dominance-weighted (lead ≥60% → 2 lead + 1 minority rows · balanced → 2+2, card shows 4 chips · single face → 3), each row wearing a bronze 汉字 god tag (`.el-godtag`, Noto Serif SC) when the set spans both faces; the face-split line moves up to become the ledger PREAMBLE ("…with a streak of The Horizon (偏财 26%). Both mark your Action:"), keeping the Domains pointer form while the minority cell is unbatched; card chips stay bare (rung law) and equal the ledger words. define/advise ¶s stay the lead god's voice. Verified: exemplar Wood 2+1 (正财×2 + 偏财 Wide-branching, tags render), Metal pointer-form fallback, Earth single-face. *(Amended same day from the live screenshot: the preamble line and the god tags both retired — "too explicit" + ten-god vocabulary is first taught in THE DOMAINS; the weighted blend renders unlabeled, `.el-facesplit`/`.el-godtag` CSS removed.)* |
| §AM.11 the three-doors reading page | 2026-09-03 | The function detail's final anatomy (owner rulings ×4, the humanization re-pass): title claim → the chip PILL ROW (scannable summary, back on the reading page by owner ruling) → define ¶ → the weighted ledger where each row is a full-sentence door variant (trait / scene / outside entries, station-backlogged ×3 per keyword, position-rotated so no two adjacent rows enter the same way) → advise ¶ → dips. The fragment-stack prose retired ("A working principle." diagnosed by owner); every example is now used inside a sentence that says what it proves. Verified on exemplar Wood (trait→scene→outside doors across the 正财/偏财 blend) and Earth friction. |
| §AM.11 takeaway strip retired | 2026-09-04 | Owner: "I'd argue we could remove this zoner" — agreed and removed. The catalogue's WHAT YOUR CHART SAYS FIRST strip (rung ① of the 2026-08-19 distillation ladder: pattern line + deep-seat teaser, deep-linked) is retired: its hook job is now done by richer rungs (covers, FUNCTION tiles, reading pages), and its deep-seat line duplicated the verdict register one screen above the wheel. Derivations (`patternSeat`/`deepSeat`/`goSeatDeep`) and `.jtakeaway` CSS removed; catalogue flows identity → core line → wheel. The position teaser corpus is untouched (still serves the Domains seats). |
| §AM.11 THE CTA IS THE THUMBNAIL | 2026-09-04 | Owner rulings ×3 on the dot cover: ① role-pill de-dup — the top-left role chip retired from the shared thumbnail (both cover and element header); the role speaks ONCE, as the equation caption's conclusion ("EARTH FEEDS METAL = ↓ FRICTION") ② the function claim renders ON the art (cover mode only — serif light-on-dark `.el-artclaim`; the element page keeps the claim as THE FUNCTION tile's title) ③ the three keyword chips return to the cover, on the art (`.el-artchips`, light pills) — the cover IS one tappable thumbnail (`.wp-thumbcta`, arrow riding the corner; `wp-codex` retired, sweep selector updated). The verdict left the cover: it lives on the FUNCTION tile and now ALSO opens the reading page under the title. Rung ladder now: cover = art + claim + bare chips · tile = claim + verdict + bare chips · reading = claim + verdict + glossed ledger. |
| §AM.11 FUNCTION tile verdict | 2026-09-04 | Owner ruling: the FUNCTION tile's meaning line is the AUTHORED `cta_verdict` (teaser register) instead of the derived first sentence of the define — the derived teaser duplicated the reading's own opener on tap-in; now the tile hooks and the detail opens fresh. The verdict also lives on the transient dot cover; the tile is its standing home. The `fnTeaser` derivation removed from journeyData. |
| §AM.11 THE THREE-BEAT CATALOGUE (journey restructure) | 2026-09-10 | Owner-ruled via mockups v3. The catalogue reads in the classical decoding order (日主 → condition+喜忌 → relations): beat ① hero only (identity + P4 arrow) with the jbridge "How does your energy run?"; beat ② **YOUR ENERGY MANUAL** — a cardstock tile "«Element» is your «Function» · it runs «CONDITION pill»" that unfolds to the §5c defline + SEEK THESE ↑CATALYST / EASE THESE ↓FRICTION row groups (ik-crow rows reused, function nouns, one-line condition whys); beat ③ **YOUR ENERGY MAP** — the dominance wheel DESIGN UNCHANGED (owner hard constraint), gaining only the SEEK/EASE legend rails (mr-dot circles, `.self` ring) and a Cinzel function label under each node (`.fnlab`). The old seek/skip panels + folio deleted; the core element itself sits on the EASE side when Overfueled, tagged YOURSELF (mirrored to SEEK when Underfueled). |
| §AM.11 stem chips retired → the stem-literacy line | 2026-09-10 | `stem_keywords` chips leave reveal plate, catalogue hero, and share card (owner: too arbitrary/generic). Replacement is the derived carved line `庚 · YANG METAL` (reveal `.rvl-lit`, share `.sc-lit`) and `庚 · YANG METAL · CAST FROM YOUR DAY OF BIRTH` on the hero (`.id-lit`) — Cinzel letterspaced, teaching glyph + polarity + element; the manifesto carries identity alone. |
| §AM.11 DM chapter = identity only + handoff | 2026-09-10 | P4 re-framed causal: overview label → "The Sign"; gifts head **{ARCH}, RUNNING RIGHT** (bridge "These light up when your core runs balanced."), shadows head **{ARCH}, RUNNING OVER/THIN** with band-derived fed-by bridges (never equated to the friction energies); chapter closes with the `dm-handoff` card ("The Blade is cast in Metal — and Metal is your Body. Your manual shows how it is running." →) returning to the catalogue manual. Body page untouched; vocabulary firewall: DM chapter speaks stem language only. |
| §AM.11 the axis homing | 2026-09-03 | Owner structure ruling: the reading's three movements assemble across two axes — define ¶ from ELEMENT_PAIR (family-true, one paragraph serving both 正/偏 siblings), keyword ledger from ELEMENT_GOD (dominance-weighted, door-rotated), advise ¶ from ELEMENT_PAIR (new `advise_*` fields). Two-faced elements need no anchor (pair prose is face-agnostic); the 50/50 case dissolves. Self-pair 金_金 keeps the "running thin/over" convention (v3 ¶0/¶2 harvested). Charts on unbatched pairs keep the v3 3-¶ fallback. Verified: exemplar Wood (merged family define over the two-god ledger), Metal core, 甲 fallback. |
| §AM.11 P4 v3 = quintessence + the carry card | 2026-09-15 | Owner-ruled via the P4 mock v3 (Current/Proposed toggle) and the audited ten-stem remap. P4 anatomy: The Sign → Your nature → **{ARCH}, RUNNING RIGHT** (bridge "Through the energies your chart asks you to seek.") ×3 gifts chosen through the chart's catalysts, each wearing a DOOR MARK (`.dm-door`: the door's element dot in map color + the manual's arrow ▲ seek / ▼ ease + the element word) → **{ARCH}, RUNNING OVER/THIN** (bridge "Through the energies already carrying weight.") ×3 shadows through the frictions, core first → the pool note ("Six of the Blade's fourteen. Your chart picks which show.") → **HOW YOUR CHART CARRIES THE {ARCH}** (`.dm-carry`: lead line · the five-energy track with arrows, the same object as the energy hero · one EASE row (core tagged you + frictions) and one SEEK row (catalysts), each = 汉字 dots + `Role · El · Fn · pct` labels + the pair `carry` clauses joined in manual order + the remedies + the trait chips it produces) → handoff "Your manual holds the full seek and ease rows. →" (replaces the dm-handoff sentence). Balanced charts: pool-order ×3+×3 under the balanced bridges; the carry card collapses to lead + track + handoff. Vocabulary firewall unchanged (no ten-god name on P4; function nouns are the hinge). `read-elemental` RETIRED: aliased to the journey's core energy page (its stem-axis sections = Codex ore). |
