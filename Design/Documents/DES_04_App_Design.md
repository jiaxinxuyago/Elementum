...
2. **Dot card · the minimal hook** *(amended by owner ruling 2026-09-01)*: rung ② is a HOOK, not a summary — the card carries exactly four layers: (a) band identity (the element's ink wash + name + 汉字, replacing the old identity row), (b) the 生/克 mechanism graphic (presence-filled capsules + law glyph + equation — the mechanism brief, carried visually), (c) the DEFINITION + VERDICT pair (owner formula 2026-09-01, superseding the derivation claim): a display definition speaking the §5f function claim ("Wood is your **Action**", role pill inline as the card's only state signal) over one verdict sentence in the position-teaser register explaining what the definition means (`ELEMENT_PAIR.cta_verdict` ×25, ≤30w, behavioral truth + one tendency-framed consequence beat — "Pressure is your forge, and the heat you choose on purpose finishes an edge that comfort would have left dull."), (d) three role-conditioned keyword chips. The deep-seat hint moved from a body line into the CTA's subtext ("starts at {seat term}"), so the hook names a concrete destination. **Retired from the card:** the family line, the state verdict sentence, the presence track, the separate identity row, the cycle image line, and the seat body line — each already covered downstream on the element page (rungs ③+). Keyword source, meaning-line form, image-line cut, and CTA-subtext placement each owner-ruled 2026-09-01.
