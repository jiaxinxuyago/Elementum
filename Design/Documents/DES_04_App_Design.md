...
| §AM.11 header + mechanism card + black arrows | 2026-09-02 | The page header (thumbnail + diagnosis) is a pure header, no longer a tap target; THE MECHANISM gets its own section card (title + base's first sentence) between the header and THE FUNCTION; the mechanism detail opens UNDER the shared thumbnail (cover = page header = detail header, one object at all three depths); and every reading tap-in speaks the readcirc language — the black arrow button, bottom-right on all three section cards. |
