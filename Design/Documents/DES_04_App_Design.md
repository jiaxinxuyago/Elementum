...
| §AM.11 diagnosis equation | 2026-09-01 | The hero card after the thumbnail = exactly two components (owner): the chemistry EQUATION graphic (core capsule state-tagged, caption concluding in the role: "EARTH FEEDS METAL = ↓ FRICTION") + ONE templated diagnosis verdict speaking the chain (state → chemistry → conclusion → directive). cta_verdict and the state-turn teaser retired from the hero (covered on the dot card / detail). |
