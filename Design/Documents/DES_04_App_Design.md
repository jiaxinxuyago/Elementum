...
| §AM.11 element hero card | 2026-09-01 | The element page's art band + mechanism card consolidated into one hero card (the energy-pill expanded card reborn): art + role chip + name → the function claim as headline → five-energy dominance track with the % beside the bar (owner refinement: number rides with the bar; rank caption removed) → capsule formula → cta_verdict → seek/skip diagnosis; taps into the mechanism detail. Tile hook + tag retired (ENERGY_TILE ore). Page = hero + Domains. |
