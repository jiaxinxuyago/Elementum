...
### P7 — the share-flow round (2026-07-22/23): card locked · footer glossary · tile redesign
Built in `share-flow.html` (supersedes p6-journey for everything on/after the catalogue; p6-journey remains the spec for Naming + Dissolve).
- **Shareable Identity Card LOCKED — the Tiles design** (Wheel & Blueprint variants retired, kept dormant for rollback). Anatomy: ELEMENTUM · YOUR IDENTITY eyebrow → locked wheel at 0.68 (day-master painting center, live dots + pips) → walnut archetype → manifesto (no italics) → keyword chips → core line (Metal is your Core + OVERFUELED pill, metal-tinted plate) → Catalyst | Friction twin tiles (relation noun + vertical dominance bar + %; Fire empty at 0%). 300×544 preview / 540×960 export; archetype-only, no personal name; QR hook dropped from the locked card. Share rail: Instagram (Story composer hand-off, mocked in-prototype), TikTok, X, Save image, Copy link. Entries: hero share button · wheel seal · float's "Read the full Day Master →".
- **Footer note chips + glossary sheet (new).** The catalogue ends on THE WORDS ON THIS PAGE · TAP ONE with the four taught-word chips (Core · Overfueled · ↑ Catalyst · ↓ Friction); tapping opens the codex glossary sheet — role-tinted hero band (t-core ink / t-cond bronze / t-cat green / t-fric red), rotated watermark icon, chip at 1.16, locked plain-language definition, Codex hand-off row. The Folio's ?-circle routes to the same Codex destination. The seal dock is KEPT (owner call 2026-07-23) — sticky above the tab bar with the dock↔pill hand-off; the footer chips sit at the scroll end beneath the towers.
- **Energy tile REDESIGN.** Collapsed: element name · relation noun · mark · dominance fill · %. Expanded, locked order: element sign + name (Fire adds · Missing) → 5-segment bar + % → definition line ("Metal is Your Core") → one-line explanation → diagnosis ("Your Metal is Overfueled — Channel it.") → role chips (Metal = Core + ↓ Friction; Fire's ↑ Catalyst carries the major ring) → three pole-tinted keyword-adjective chips → read circle. Retired: corner role ring, sp-hook glance label, pole-noun verdict, flavor line. Engine flag: diagnosis is now per element — energyRoles.js must expose per-element condition + remedy.
- **Wheel:** visuals unchanged (locked); the written law expanded into a six-line anatomy ledger on the journey sheet + a full breakdown board (geometry rule · seating law · pips · seal · intro motion · doors).
- **Handoff boards updated:** all pill instances rebuilt to the new tile design; new boards 04 (identity card), 05 (footer chips + glossary sheet), 06 (full iconography legend incl. share rail, platform glyphs, codex, ?-circle, taught-word chips), 07 (dominance wheel); footer chips added to the full-scroll board (C7); element thumbnail art-direction sheets embedded (art/boards/thumb-*.png). The five pill-open-*.png captures are stale (previous design) — live frames on p6-breakdown-tiles.html are current.
- **Sheet chrome:** sticky page switcher (.omnav) across the three sheets — design-doc navigation only, never app UI.
