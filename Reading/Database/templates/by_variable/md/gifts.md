# `gifts`  ·  by-variable comparison

> **GENERATED from the by_axis JSON — do not hand-edit.** Edit the JSON (or request the change), then re-run `node tools/build-template-twins.mjs`.
>
> GENERATED comparative pivot of the by_axis station (registry fields only; __ore and TEMPLATED excluded). Truth = by_axis/json — edit there, then re-run node tools/build-template-twins.mjs.

## STEM (×10)

### The Oak · 甲

| Field | Value |
|---|---|
| `0.phrase` | Plants a stake |
| `0.dim` | commitments |
| `0.door` | action |
| `0.desc` | Ambition lands somewhere real with you. The idea becomes a lease, a title, a name on the door, while other people are still describing theirs. |
| `1.phrase` | Raises everyone's game |
| `1.dim` | influence on others |
| `1.door` | expression |
| `1.desc` | People around you end up aiming higher than they meant to. Your reach quietly becomes the new ceiling, and nobody remembers deciding that. |
| `2.phrase` | Builds to last |
| `2.dim` | at work |
| `2.door` | order |
| `2.desc` | You can't put real effort into something disposable. What you build is still standing years later, long after the quick versions fell down. |
| `3.phrase` | Keeps the faith |
| `3.dim` | inner life |
| `3.door` | body |
| `3.desc` | You still believe in where it's all heading, even in seasons when nothing looks like proof. That belief is what gets things through winter. |
| `4.phrase` | Late bloomer |
| `4.dim` | over the years |
| `4.door` | mind |
| `4.desc` | The Oak that everyone wrote off in spring is the one still standing in October. Your growth arrives on its own calendar, and it arrives. |
| `5.phrase` | Holds its ground |
| `5.dim` | standing alone |
| `5.door` | body |
| `5.desc` | Wind from every side, and you are still where you were planted, still facing the same way. |
| `6.phrase` | Takes the lesson |
| `6.dim` | hard seasons |
| `6.door` | mind |
| `6.desc` | One hard season teaches you what a decade of easy ones could not, and you keep it. |

### The Vine · 乙

| Field | Value |
|---|---|
| `0.phrase` | Nobody's fool |
| `0.dim` | reading people |
| `0.door` | mind |
| `0.desc` | You clock the difference between what's said and what's meant almost instantly. People stopped managing you long ago. It never worked. |
| `1.phrase` | Finds another way |
| `1.dim` | obstacles |
| `1.door` | action |
| `1.desc` | Where others push the locked door harder, you're already around the back. You arrive where you meant to, by a path nobody predicted. |
| `2.phrase` | Wins people over |
| `2.dim` | trust |
| `2.door` | expression |
| `2.desc` | Not loudly, and not fast. But give it a season and the skeptics are asking your opinion first. Trust built your way tends to hold. |
| `3.phrase` | Attentive friend |
| `3.dim` | close bonds |
| `3.door` | body |
| `3.desc` | You track what the people around you need without being told. The birthday, the hard week, the thing they mentioned once in passing. You caught it. |
| `4.phrase` | Quiet survivor |
| `4.dim` | hard seasons |
| `4.door` | order |
| `4.desc` | The storms that snapped stronger-looking people somehow didn't end you. You bent, waited, and grew back. It's a longer game than anyone realizes. |
| `5.phrase` | Regrows from little |
| `5.dim` | setbacks |
| `5.door` | body |
| `5.desc` | Cut back to nothing in March, and by June there is a green line up the wall again. |
| `6.phrase` | Maps the wall |
| `6.dim` | second attempts |
| `6.door` | mind |
| `6.desc` | Every wall you have climbed is a map you keep. The second time is never the long way. |

### The Sun · 丙

| Field | Value |
|---|---|
| `0.phrase` | Founds the tradition |
| `0.dim` | what lasts |
| `0.door` | expression |
| `0.desc` | The dinner you hosted once is now the dinner everyone hosts. What you warm tends to set into something people keep doing after you have moved on. |
| `1.phrase` | No hidden agenda |
| `1.dim` | hard days |
| `1.door` | body |
| `1.desc` | Your warmth reads as real because it is. It showed up on the worst days too, when a performed version would have flickered out. |
| `2.phrase` | Hard problems only |
| `2.dim` | the work you pick |
| `2.door` | action |
| `2.desc` | Easy work makes you careless. Hand you the negotiation nobody wants or the client everyone dreads, and the heat finally has something worth working on. |
| `3.phrase` | Keeps it close |
| `3.dim` | close bonds |
| `3.door` | body |
| `3.desc` | The wide broadcast costs more than it used to, so the warmth goes where it counts now. The few in your light feel it fully. |
| `4.phrase` | Feeds on daylight |
| `4.dim` | refueling |
| `4.door` | mind |
| `4.desc` | You feed on what is around you. A good morning, a good friend, a good idea, and you are lit again. |
| `5.phrase` | Catches the mood |
| `5.dim` | reading a mood |
| `5.door` | mind |
| `5.desc` | You know the weather of a place the moment you walk in, and you adjust your light to it. |
| `6.phrase` | Bright under pressure |
| `6.dim` | bad days |
| `6.door` | order |
| `6.desc` | The harder the day, the steadier the shine. People stand nearer to you when things go wrong. |

### The Candle · 丁

| Field | Value |
|---|---|
| `0.phrase` | Deeply present |
| `0.dim` | one-on-one |
| `0.door` | body |
| `0.desc` | When you're with someone, you're entirely with them. Phones stay dark, time goes quiet, and they leave feeling seen in a way that's hard to find. |
| `1.phrase` | Misses nothing |
| `1.dim` | details |
| `1.door` | mind |
| `1.desc` | The small tell, the changed tone, the detail everyone walked past. You caught it, and three months later it turned out to be the whole story. |
| `2.phrase` | Fully committed |
| `2.dim` | the inner circle |
| `2.door` | body |
| `2.desc` | What you choose gets everything you have. The people and work inside your circle never have to wonder where they stand. |
| `3.phrase` | Rekindles people |
| `3.dim` | influence on others |
| `3.door` | expression |
| `3.desc` | Your belief in someone works at close range. A friend arrives half given up, and somewhere over tea at your table, the pilot light catches again. |
| `4.phrase` | Keeps the flame |
| `4.dim` | hard seasons |
| `4.door` | order |
| `4.desc` | The light is smaller these days, but it hasn't gone out. Through stretches that extinguished louder fires, yours kept burning, quietly, all night. |
| `5.phrase` | Studies one thing |
| `5.dim` | expertise |
| `5.door` | mind |
| `5.desc` | One subject, years deep. You know it the way people know a face. |
| `6.phrase` | Tends what's near |
| `6.dim` | small work |
| `6.door` | action |
| `6.desc` | The small work in arm’s reach gets done well and on time. That is where you build. |

### The Mountain · 戊

| Field | Value |
|---|---|
| `0.phrase` | Everyone's rock |
| `0.dim` | being leaned on |
| `0.door` | body |
| `0.desc` | Friends plan around you the way builders plan around bedrock. When their lives shake, yours is the number they dial with their hands still trembling. |
| `1.phrase` | Shows up regardless |
| `1.dim` | showing up |
| `1.door` | body |
| `1.desc` | Storms, funerals, moving days, the long boring middle of hard times. The dramatic helpers come and go. You're still there in week six. |
| `2.phrase` | Calm under fire |
| `2.dim` | crisis |
| `2.door` | order |
| `2.desc` | The news that spins everyone else barely moves your needle. People watch your face in a crisis to decide how scared to be. |
| `3.phrase` | Safe harbor |
| `3.dim` | secrets |
| `3.door` | mind |
| `3.desc` | People tell you things they've told no one. Nothing shocks you, nothing leaks, and nothing changes how you treat them the next morning. |
| `4.phrase` | Slow and sure |
| `4.dim` | pace |
| `4.door` | action |
| `4.desc` | You move at your own tempo and it's not fast. But what you finish is finished, and what you promised happens, eventually, completely. |
| `5.phrase` | Keeps the record |
| `5.dim` | memory |
| `5.door` | mind |
| `5.desc` | You remember what was said, when, and by whom. Not to use it. Because it happened. |
| `6.phrase` | Says it once |
| `6.dim` | speaking up |
| `6.door` | expression |
| `6.desc` | You say the thing one time, plainly, and do not repeat it. People learn to listen the first time. |

### The Field · 己

| Field | Value |
|---|---|
| `0.phrase` | Anticipates needs |
| `0.dim` | unspoken needs |
| `0.door` | mind |
| `0.desc` | The tea appears before the cough. The help arrives before the asking. You noticed what someone needed a full day before they did. |
| `1.phrase` | Grows people |
| `1.dim` | others' growth |
| `1.door` | expression |
| `1.desc` | People become better versions of themselves around you and credit themselves entirely. They never see the conditions you built under their feet. You prefer it that way, mostly. |
| `2.phrase` | Keeps every promise |
| `2.dim` | promises |
| `2.door` | order |
| `2.desc` | Your care doesn't have seasons of convenience. What you said in spring still stands in winter, and people build their lives on that. |
| `3.phrase` | Restful company |
| `3.dim` | comfort |
| `3.door` | body |
| `3.desc` | Nobody performs around you. Friends unclench, say the true version, and stay an hour longer than they planned. Your presence works like fallow ground. |
| `4.phrase` | Quietly essential |
| `4.dim` | being noticed |
| `4.door` | body |
| `4.desc` | Nothing about you announces itself. But when you step back for a while, everything wilts a little, and everyone finally notices who was watering. |
| `5.phrase` | Reads the season |
| `5.dim` | timing |
| `5.door` | mind |
| `5.desc` | You know when to plant and when to leave the ground alone. Timing is the thing you never explain. |
| `6.phrase` | Feeds the table |
| `6.dim` | providing |
| `6.door` | action |
| `6.desc` | There is always enough. Nobody asks how. You made sure before they were hungry. |

### The Blade · 庚

| Field | Value |
|---|---|
| `0.phrase` | Crisis performer |
| `0.dim` | under pressure |
| `0.door` | order |
| `0.desc` | The day everything breaks is the day you are calmest. Others freeze. You are already cutting the problem into pieces that can be solved. |
| `1.phrase` | First-handshake read |
| `1.dim` | first impressions |
| `1.door` | mind |
| `1.desc` | Ten minutes into meeting someone, you know who they are. Whether you say so is another matter. Months later everyone else catches up to the handshake. |
| `2.phrase` | No dressed-up answers |
| `2.dim` | communication |
| `2.door` | expression |
| `2.desc` | When you do speak, you say the thing the table has been talking around. People save their hardest questions for you because the answer comes back plain. |
| `3.phrase` | Quiet loyalty |
| `3.dim` | close bonds |
| `3.door` | body |
| `3.desc` | Strangers get the edge. The few people you've let past it get someone who shows up at 2 AM without being asked, every time. |
| `4.phrase` | Careful finisher |
| `4.dim` | at work |
| `4.door` | body |
| `4.desc` | You take longer to call something done, and it shows. What leaves your hands doesn't come back for fixing. |
| `5.phrase` | Clears dead weight |
| `5.dim` | trimming a plan |
| `5.door` | action |
| `5.desc` | Handed a plan with twelve moving parts, you find the three that matter and the nine that were there for comfort. |
| `6.phrase` | Practices alone |
| `6.dim` | practice |
| `6.door` | mind |
| `6.desc` | The skill arrives finished. Nobody saw the quiet months of practice behind it, and you like it that way. |

### The Jewel · 辛

| Field | Value |
|---|---|
| `0.phrase` | Impeccable taste |
| `0.dim` | quality |
| `0.door` | mind |
| `0.desc` | You know the good one from the almost-good one at a glance, the way some people can tell a note is flat. Friends borrow your eye before they buy. |
| `1.phrase` | Quality guaranteed |
| `1.dim` | at work |
| `1.door` | order |
| `1.desc` | Nothing leaves your hands that you don't believe in. Years later your work is the one still holding up under inspection. |
| `2.phrase` | Spots the crack |
| `2.dim` | early warnings |
| `2.door` | mind |
| `2.desc` | You saw the flaw three months before it became the problem. You say it out loud exactly once, and then wait. |
| `3.phrase` | Notices the effort |
| `3.dim` | others' work |
| `3.door` | body |
| `3.desc` | The detail someone worked hardest on, the one everyone else missed, is the first thing you praise. Being appreciated by you means something because you actually looked. |
| `4.phrase` | Understated excellence |
| `4.dim` | self-presentation |
| `4.door` | expression |
| `4.desc` | You don't advertise, compete, or explain. The work speaks quietly, the taste shows in small choices, and the people who know, know. |
| `5.phrase` | Keeps its shape |
| `5.dim` | being handled |
| `5.door` | body |
| `5.desc` | Handled by everyone, changed by no one. You come out of every year the same stone. |
| `6.phrase` | Prices it right |
| `6.dim` | placing value |
| `6.door` | action |
| `6.desc` | You know what a thing is worth and where it belongs. Placed by you, it stays placed. |

### The Ocean · 壬

| Field | Value |
|---|---|
| `0.phrase` | Big picture thinker |
| `0.dim` | connections |
| `0.door` | mind |
| `0.desc` | While the meeting argues about one tile, you're seeing the whole mosaic. How it connects, where it's heading, what it touches three steps out. |
| `1.phrase` | Sharpened by rigor |
| `1.dim` | good teachers |
| `1.door` | mind |
| `1.desc` | Vague knowing becomes usable the moment someone exact gets hold of it. A rigorous teacher, a clean method, and the depth finally has a shape to pour through. |
| `2.phrase` | Thinks big |
| `2.dim` | horizons |
| `2.door` | action |
| `2.desc` | Small plans bore you on arrival. You're drawn to the far shore, the decade plan, the version of the idea that scares the committee. |
| `3.phrase` | Even keel |
| `3.dim` | under pressure |
| `3.door` | order |
| `3.desc` | Panic passes over you like weather over open water. Surface ruffles, depth undisturbed. People borrow your calm in their worst hours. |
| `4.phrase` | Seeds the idea |
| `4.dim` | what grows after you |
| `4.door` | expression |
| `4.desc` | You mention it once at dinner and walk away. A year later it is someone's company, and they are only half sure where it came from. |
| `5.phrase` | Nothing rattles it |
| `5.dim` | storms |
| `5.door` | body |
| `5.desc` | Storms cross the surface. Ten fathoms down, nothing has moved. |
| `6.phrase` | Keeps its level |
| `6.dim` | recovery |
| `6.door` | body |
| `6.desc` | Whatever pours in, you find your level again by morning. |

### The Rain · 癸

| Field | Value |
|---|---|
| `0.phrase` | Feels the undercurrent |
| `0.dim` | unspoken feelings |
| `0.door` | mind |
| `0.desc` | You knew the couple was fighting before they did. The mood under the smiles reaches you first, ahead of any evidence. |
| `1.phrase` | Reads the moment |
| `1.dim` | timing |
| `1.door` | action |
| `1.desc` | You felt the turn before the numbers did. The launch, the offer, the door about to open: you were already there when the bright thing arrived. |
| `2.phrase` | Gentle confidant |
| `2.dim` | secrets |
| `2.door` | body |
| `2.desc` | People hand you their fragile things. The secret, the shame, the half-formed hope. Something about you promises soft handling, and you've never broken it. |
| `3.phrase` | Changes minds softly |
| `3.dim` | influence on others |
| `3.door` | expression |
| `3.desc` | Nobody remembers losing the argument to you. Positions just shift a degree a week until, months later, everyone agrees and thinks they always did. |
| `4.phrase` | Settles the dust |
| `4.dim` | presence |
| `4.door` | body |
| `4.desc` | The air is calmer after you have been in it, and nobody can say what you did. |
| `5.phrase` | Soaks it up |
| `5.dim` | learning |
| `5.door` | mind |
| `5.desc` | Whatever falls near you goes in and stays. Years later it comes back as knowing. |
| `6.phrase` | Wears down stone |
| `6.dim` | under pressure |
| `6.door` | order |
| `6.desc` | Pressure does not stop you. Given enough seasons, the rain shapes the rock that tried to stop it. |
