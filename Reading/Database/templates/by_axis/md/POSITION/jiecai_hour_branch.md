# 劫财在时支 — The Rival inside the Hour Gate  ·  POSITION archetype
...
| **axis** | POSITION |
| **key** | 劫财在时支 |
| **canonical name** | The Rival inside the Hour Gate |
...
| Variable | Value |
|---|---|
| `term` | The Rival inside the Hour Gate |
| `term_zh` | 劫财在时支 |
| `gate` | Hour Gate |
| `slot_zh` | 时支 |
| `slot_kind` | branch |
...
| `domain_readings.Family` | Your late family life is vivid: heirs with your fire and their own compass, a household that debates because it cares. Do not mistake their independence for distance. The Rival’s children love by contending. Set the table anyway, referee lightly, and the noise turns out to be the warmth. |
| `domain_readings.Wealth` | Late wealth needs a gatekeeper, and it should be you on your clearest day: claims will come dressed as family, opportunity, and honor. Fix your giving number annually, protect the floor beneath your own feet first. Generosity from a defended base reads as blessing. From an open vault it reads as weather. |
