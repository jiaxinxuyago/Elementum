# tpl_dm_prescription  ·  TEMPLATED archetype

> **GENERATED from the by_axis JSON — do not hand-edit.** Edit the JSON (or request the change), then re-run `node tools/build-template-twins.mjs`.
>
> All variables below are CANDIDATES carried from the live corpus / REA_02 locks — the axis construct is TBD, ruled per-axis with the owner.

| | |
|---|---|
| **axis** | TEMPLATED |
| **key** | tpl_dm_prescription |
| **construct** | TBD — ruled per-axis with the owner |
| **status** | INTERIM (pending K2) — UNSURFACED since the J4 sub-screen retired (owner 2026-08-13); P4 renders no prescription cards |
| **budget** | 1–2 sentences |
| **sources** | REA_03 §5 (patterns) |

## Candidate variables

| Variable | Value |
|---|---|
| `pattern` | SEEK THIS · {EL} / SKIP THIS · {EL} + body |
| `clauses.seek_present` | {El} is the energy your chart asks for — thin in you and worth feeding. Seek it on purpose. |
| `clauses.seek_missing` | {El} is the energy you don't carry — the one your chart asks for most. Borrow it daily{, through …}. |
| `clauses.skip` | {El} is already rich in you — more of it weighs the core. Stop adding; let what you have ease. |
