...
1. **`Reading/Database/templates/by_axis/json/`** — one **JSON file per data template** (per REA_03 variable): the authoring and review station and the ONLY editing surface. Seeded from the live corpus for LIVE/INTERIM variables; stubbed (keys enumerated, values null) for PLANNED ones. Each JSON carries a header block (`$archetype`, `axis`, `key`, `canonical_name`, `construct`, `candidate_note`, `sources`, + `status_note` where set) so a file is self-describing. The seeder also emits `_ORDER.json` — the canonical archetype order per axis, which fixes variant order in the pivot.
...
- **Archetype files:** `$archetype` + `axis` + `key` + `canonical_name` + `construct` (TBD marker) + `candidates` (+ `__ore`). The per-axis variable CONSTRUCT is decided in the owner-supervised data rulings; until then every value is a candidate.
...
| `Elementum_App/tools/build-template-twins.mjs` | Regenerates every generated view from the by_axis JSONs: the `by_axis/md` twins, the `by_variable/{json,md}` pivot AND `templates/corpus_review.html`. Run after any JSON edit. |
...
| `Elementum_App/tools/voice-audit.mjs` | **The REA_16 mechanical tone audit, v2 registry-driven** (2026-08-05): parses the REA_16 §2c AUDIT REGISTRY live and enforces each row's register rules over the surface it names — every station axis + declared code content (daily guidance). `locked`/`live` rows block (`VOICE`, exit 1); `pending` rows report informationally (`PEND`) so the unauthored ~80% is visible, never blocking; an authored station field with NO registry row is itself a violation (the expansion contract — new variables register in §2c first, zero tool edits). The human half (swap test, angle fidelity, read-aloud) stays human. |
