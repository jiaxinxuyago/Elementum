...
- **Selection (derived, code `selectPoolByDoor`):** gifts = one per catalyst energy in the manual's SEEK order, the third slot filled from the heaviest catalyst's second item; shadows = one per friction energy, core first, in the manual's EASE order, filled from the heaviest friction. Balanced charts (rails hidden): pool order ×3 + ×3 under the balanced bridge.
