...
| **C4** | tgPattern cardinality | **REA_01/REA_04 5-pattern canonical**; Bible's 7-value scheme **superseded**. |
| **C5** | REA_05 §9 doctrine | **Exception carved** — energy-card content **does** vary by presence frame. |

---

## 3 · Per-document edit list (to execute on go-ahead)

### Reading schema → bump to v2.1 (`REA_04_Reading_Generation_Schema.md`)
...
- **REA_04**: re-scope `dominantEnergy.characterDesc` from element-scoped to **persona-scoped**; add the **`rulingDomain`** axis (×50, DM-relative), distinct from the Western `lifeDomain` buckets.

### Generation
- **REA_05**: retarget Pipeline A2 → `ENERGY_CARD_DATA[element_god]` ×50 with presence-frame registers (dominant+absent bespoke, present derived) + `rulingDomain`; **carve the §9 exception** (C5).
- **REA_06**: add authoring sections — per-face persona prompt · register-variation prompt · ruling-domain prompt.
...
- `Reading Schema.pdf` → "SUPERSEDED by REA_04_Reading_Generation_Schema (v2.1)."
...
- `BaZi_Analysis_Bible` 7-pattern → "Superseded by REA_01/REA_04 5-pattern."
