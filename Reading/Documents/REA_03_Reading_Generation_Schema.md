...
| `gifts` (tagged pool) | phrase + desc ×~5 | STEM + band tags | — | **TAGGED-POOL construct (owner-ratified 2026-08-05):** ~5 items/stem, each `{phrase, desc, bands}`; resolver selects ×3 per chart band (band-tagged first, all-band fill); band-robust wording law on all-band items; supersedes the fixed ore ×3 at the §7 #4 rewrite | — (ore ×3 renders interim on P4) | P4 Gifts & Shadows panel | PLANNED — band-tier campaign block B |
| `shadows` (tagged pool) | phrase + desc ×~5 | STEM + band tags | — | same construct, symmetric with `gifts` (owner: if one varies, both vary) | — (ore ×3 renders interim on P4) | P4 Gifts & Shadows panel | PLANNED — band-tier campaign block B |
