...
| `POSITION` axis: `term`/`term_zh` (derived) + `domains` + `defline` + `reading` | named configuration ×70 (god × slot; day stem excluded) | POSITION (god×gate·slot) | defline ≤25w · reading 60–95w | **Named events (REA_02 §5e, owner 2026-08-19):** "The Alchemist inside the Career Gate · 偏印在月支"; reading DECLARES its 2–3 ruled domains from the canonical taxonomy ×8 (Wealth·Health·Career·Love·Family·Social·Mind·Growth) in sentence one, then interprets god × gate × slot-kind | This position rules Mind and Growth, and it rules them from the Career Gate… | surfacing PENDING (engine resolves each chart's positions → element pages / P5; slot ruling at wiring) | **CORPUS LOCKED ×70 (owner 2026-08-19; template pianyin_month_branch approved, batch authored to it)** |
