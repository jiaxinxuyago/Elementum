...
| `gifts` (tagged pool) | phrase + desc ×5 | STEM + band tags | phrase 1–2w | **TAGGED-POOL construct (owner-ratified 2026-08-05; dimension law v3 2026-08-14):** 5 items/stem, station `{phrase, dim, desc, bands}` → code `{phrase, desc, bands}`. **phrase = the TRAIT (1–2 common personality words) · `dim` = solely the ANGLE (the life-facet lens, station-side, unique per pool, ≠ phrase — audit-enforced) · desc = the trait seen through that angle.** `selectPoolByBand` picks ×3 per chart band (band-tagged first, all-tagged in pool order); REA_16 §3 pool laws v3 | Composed `dim: under pressure` The day everything breaks is the day you're calmest… `bands:[concentrated]` | P4 Gifts & Shadows panel + d12 Elemental Nature detail | **LIVE · pools LOCKED ×10 (owner; v3 re-author 2026-08-14)** |
| `shadows` (tagged pool) | phrase + desc ×5 | STEM + band tags | phrase 1–2w | same construct, symmetric with `gifts` (owner: if one varies, both vary) | Indecisive `dim: decision-making` You reach an answer, then reopen it… `bands:[open]` | P4 Gifts & Shadows panel + d12 Elemental Nature detail | **LIVE · pools LOCKED ×10 (owner; v3 re-author 2026-08-14)** |
