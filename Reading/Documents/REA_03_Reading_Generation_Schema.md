...
| `k2_overview` + `k2_functional` ×5 + `k2_domain_readings` | paragraph + 5 lines + per-domain ¶ | ELEMENT·GOD ×50 | 45–70w · ≤22w each · 30–55w each | **K2 CELL (owner construct 2026-08-19):** overview = the "what it means" body (replaces the interim `mean_line`) · functional = Mind/Expression/Bonds/Action/Body · domain readings = one per the god's ruled domains (`GOD.domains` ×3 nouns, LIVE free chips), **Seeker-gated**; K2 register (god cost × element arena); palace layer (pillar placement line) PENDING its palace-noun vocabulary ruling. Supersedes `k2_seeker_depth`. | 土_偏印 template: "Your Earth carries the Alchemist's current…" | element page (journey sub-screen; app-energy = future full home) | **TEMPLATE CELL LIVE (土_偏印, awaiting owner lock) · ×49 PLANNED** |
