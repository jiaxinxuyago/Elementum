...
| `dm_overview` | paragraph | STEM ×10 | 55–85w | **THE SIGN paragraph (owner SIGN/PERSON division 2026-08-14):** teaches the BaZi term + builds the symbolic persona image (name → term → season/cycle meaning → image as a figure); third person, material/metaphor domain ONLY — no user psychology (that is `yourNature_desc`'s exclusive job; the P4 pair reads symbol → decoding) · register: ANALYST'S LEDGER + humanized gate | The Blade is Yang Metal, the hard metal of the system: ore taken from the mountain, put through fire, and finished into an edge. … Its image is the general's sword kept plain and sharp… | P4 Day-Master screen (opening section) | **LIVE — v2 corpus LOCKED ×10 (owner 2026-08-14; mechanism-era corpus retired), transcribed to `identity.overview`, harvest-mapped, audit-guarded** |
