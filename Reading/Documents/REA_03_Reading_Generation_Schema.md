...
| `gifts` (tagged pool) | phrase + desc ×5 | STEM + band tags | phrase 2–3w | **TAGGED-POOL construct (owner-ratified 2026-08-05):** 5 items/stem, station `{phrase, dim, desc, bands}` → code `{phrase, desc, bands}` (`dim` = the item's 3-word trait reduction, station-side, unique per pool — audit-enforced); `selectPoolByBand` picks ×3 per chart band (band-tagged first, all-tagged in pool order); REA_16 §3 pool laws (trait-phrase / **Angle Fork law v2** / real-life-image desc) | Crisis performer. The day everything breaks is the day you're calmest. … `bands:[concentrated]` | P4 Gifts & Shadows panel + d12 Elemental Nature detail | **LIVE · pools LOCKED ×10 (owner 2026-08-13, band-tier block B — ore ×3 retired)** |
| `shadows` (tagged pool) | phrase + desc ×5 | STEM + band tags | phrase 2–3w | same construct, symmetric with `gifts` (owner: if one varies, both vary) | Endless second-guessing. You reach an answer, then reopen it… `bands:[open]` | P4 Gifts & Shadows panel + d12 Elemental Nature detail | **LIVE · pools LOCKED ×10 (owner 2026-08-13)** |
