...
> **Formerly DES_05 + DES_07** (moved to the Reading library 2026-07-23) **and before that DOC6 + DOC9** (2026-07-09). Historical citations of "DES_05", "DES_07", "DOC6" or "DOC9" refer to this file (registry: Operations/README.md).
...
| `stem_keywords` ×3 | labels | STEM | 1w each | 3 chips — 3-dimension rule, no virtues, no absolutes, no cross-stem duplicates (REA_16 §3) | Direct · Decisive · Sharp | NONE — retired from reveal + hero + share 2026-09-10 (owner) | **RETIRED · CODEX ORE (the derived stem-literacy line `庚 · YANG METAL [· CAST FROM YOUR DAY OF BIRTH]` replaced them)** |
| `inscription` | inscription | STEM **(axis LOCKED ×10 — owner 2026-08-03, same identity-seal ruling as `manifesto`)** | 12–16w · 67–81c (locked corpus) | **≤17w / ≤85c — R2 owner-ruled 2026-08-03**; DASHLESS two-beat (REA_16); target shape adds `zh` 4–6字 (aphorism register, not a translation) | You say what others soften, then quietly pay for being the one who did. | **UNSURFACED by design — home RULED (owner 2026-08-13): `dm_claims` claim 1.** The corpus stays locked and surfaces when the dm_claims block ships on P4 (claim 1 ≡ inscription). The J4 sub-screen (its old fallback surface) was retired the same day. | **LOCKED ×10 (owner 2026-08-05, REA_16 corpus — priors retired) · home = dm_claims (2026-08-13)** |
...
| `gifts` (door-tagged pool) | phrase + dim + door + desc ×7 | STEM | phrase 2–3w situational | **DOOR-TAGGED construct (owner-ruled 2026-09-15, REA_02 §5h; supersedes the band-tagged pool):** 7 items/stem, station `{phrase, dim, door, desc}` → code `{phrase, door, desc}`. `door` = the §5f function the trait is exercised through (Body · Mind · Expression · Action · Order), one gift face per function with Body and Mind doubled. Phrase law v4 · dim law v3 · desc law unchanged; every item is a MECHANISM claim that holds on any chart (tempo and channel lifted out). `selectPoolByDoor` picks ×3: one per catalyst energy in the manual's SEEK order, the third from the heaviest catalyst's second item; Balanced charts → pool order ×3 | Crisis performer `dim: under pressure` `door: order` The day everything breaks is the day you are calmest… | P4 Gifts panel (door mark = the door's element + the manual's arrow) | **LIVE · 庚 template + batch ×9 (the ten-stem remap, audited 2026-09-15)** |
| `shadows` (door-tagged pool) | phrase + dim + door + desc ×7 | STEM | phrase 2–3w situational | same construct, symmetric with `gifts`: one SHADOW face per function = the function OVERGROWN (REA_02 §4b), Body and Mind doubled. `selectPoolByDoor` picks ×3: one per friction energy, core first, in the manual's EASE order, the third from the heaviest friction. Thin-self items retired to `__ore` (underuse costs are the carry card's SEEK rows) | Stays decided `dim: second chances` `door: body` Once you have decided what something is, it stays that way… | P4 Shadows panel | **LIVE · 庚 template + batch ×9** |
...
| `yourNature_desc` (baseline ×10 + band variants ×30) | paragraph | STEM baseline + STEM·BAND variants (pattern grain RETIRED — resolver falls band → baseline) | variants 50–75w (v2) · baseline 38–50w | **v2 BRIDGE structure (owner 2026-08-14): bridge → band portrait → landing.** Sentence 1 = the You-opener BATON PASS: receives the dm_overview myth's central image onto the reader ("You are what the sword is like when it wakes up as a person") — the orchestrated third→second person transition; close lands on the person. Significance law: the decoding outweighs the myth — this is THE hook paragraph. You-opener + humanize gate + swap-gram; band = *how it's running* | conc: You are what the sword is like when it wakes up as a person… · open: You keep the edge the stories promise, under cloth and out of view… | P4 "Your nature" section, band-resolved (baseline = fallback) + d12 detail | **baseline LOCKED ×10 + v2 BRIDGE VARIANTS LOCKED ×30 (owner 2026-08-14; BAND-A v1 corpus superseded, priors in git)** |
| `self_card` (face + presence) | descriptor + paragraph | STEM·BAND ×30 | face ≤8w · presence ≤30w | face = the state named in the element's arena (situational register) · presence = being that state, dignified on the open band · swap-gram | The edge, never sheathed. / The cut is always available and mostly used… | **CORE element screen only** (`buildElementScreen` → `isCore` → `STEM_VARIANTS[stem_band].selfCard`; owner slot ruling 2026-08-14) | **LIVE · LOCKED ×30 (owner 2026-08-14, BAND-C — fresh corpus; the `IDENTITY_SATURATION_READING` predecessor never existed in code)** |
| `dm_overview` | paragraph | STEM ×10 | 55–85w | **THE SIGN paragraph (owner SIGN/PERSON division 2026-08-14):** teaches the BaZi term + builds the symbolic persona image (name → term → season/cycle meaning → image as a figure); third person, material/metaphor domain ONLY — no user psychology (that is `yourNature_desc`'s exclusive job; the P4 pair reads symbol → decoding) · **poet-narrator cadence + subject rule (owner 2026-08-14): the sign itself is the only actor — no schoolbook connectors, no unnamed narrators ("the old stories," "the ancients"); plain vocabulary** · register: ANALYST'S LEDGER + humanized gate | The Blade is Yang Metal, the metal that has already been through fire. It arrives with autumn… it does the harvest's honest arithmetic… Between wars it hangs on the wall and says nothing… | P4 Day-Master screen (opening section) | **LIVE — v2 corpus LOCKED ×10 (owner 2026-08-14; mechanism-era corpus retired), transcribed to `identity.overview`, harvest-mapped, audit-guarded** |
...
| `POSITION` axis: `term`/`term_zh` (derived) + `domains` + `defline` + `reading` | named configuration ×70 (god × slot; day stem excluded) | POSITION (god×gate·slot) | defline ≤25w · reading 60–95w | **Named events (REA_02 §5e, owner 2026-08-19):** "The Alchemist inside the Career Gate · 偏印在月支"; reading DECLARES its 2–3 ruled domains from the canonical taxonomy ×8 (Wealth·Health·Career·Love·Family·Social·Mind·Growth) in sentence one, then interprets god × gate × slot-kind | This position rules Mind and Growth, and it rules them from the Month Gate… | **SURFACED (owner 2026-08-19): FULL readings = the element page's Domains detail (seats accordions inside the per-god sub-blocks; `resolvePositions` filtered by occupying element); P5 Pillar Chart = the index (term + 汉字 + domain chips only). GATING (curation ruling 2026-08-19): seat names + deflines FREE, the full position reading = SEEKER** | **CORPUS LOCKED ×70 (owner 2026-08-19; template pianyin_month_branch approved, batch authored to it)** |
| `k2_overview` + `k2_functional` ×1 + `k2_domain_readings` | paragraph + 1 line + per-domain ¶ | ELEMENT·GOD ×50 | 40–70w · ≤22w each · 18–55w each | **K2 CELL (owner construct 2026-08-19):** overview = the "what it means" body (replaces the interim `mean_line`) · functional = ONE line per cell (construct re-ruled 2026-09-01: the god's FAMILY function under the §5f bijection, ×200 cross-function lines deleted — interactions belong to TG_PATTERN; LIVE in the Domains god sub-blocks: "runs your {Function}: {line}") · NEW `adj_chips` {catalyst ×3, friction ×3} 1–3w each (element-flavored dot-card chips, owner 2026-09-01: template ×5 approved → batch ×45, CORPUS LOCKED ×50 (uniqueness enforced within same-element sibling pairs only, core principles 2026-09-04); supersedes the god-grain ADJ tables, kept as defensive fallback) - NEW `fn_reading` {catalyst, friction} = **LEDGER-ONLY after THE AXIS HOMING (owner structure ruling 2026-09-03: define/advise ¶s live at ELEMENT_PAIR grain — the family defines the function, the god defines the character)**: ledger ×3 keywords × 3 DOORS {trait, scene, outside}, each door a full-sentence variant 35–55w (THE THREE DOORS, REA_16 §2c), assembled by position rotation. CORPUS COMPLETE ×50 (owner 2026-09-04: god-track batch ×44 + chip vocabulary re-audit ×17 swaps) (**THE GOD-GRAIN FUNCTION READING, owner 2026-09-03:** the full function reading moved to this axis so it derives from the chart's LEAD ten-god under THE KEYWORD DERIVATION CHAIN, REA_16 §2c; `adj_gloss` RETIRED unlocked, absorbed into the ledger; chips re-angled 土_偏印 Shut-away→Overprepared + 金_比肩 Tempered→Reserved; TEMPLATE ×12 pending lock (5 lead cells + 木_偏财, both poles) → batch ×88; two-faced elements = THE WEIGHTED LEDGER (owner rulings ×4, 2026-09-03: lead ≥60% → 2 lead + 1 minority rows · balanced <60% → 2+2 with 4 card chips · the blend renders UNLABELED (split line + god tags retired same day, owner: ten-god vocabulary is first taught in THE DOMAINS; rows keep their source god as code metadata) · chip rank = authored order · define/advise stay lead-god 以强者论 under the balanced-compatibility authoring law · sibling-pair audit ×25 in the batch) · domain readings = one per the god's ruled domains (`GOD.domains` ×3 nouns, LIVE free chips), **Seeker-gated**; K2 register (god cost × element arena); palace layer (pillar placement line) PENDING its palace-noun vocabulary ruling. Supersedes `k2_seeker_depth`. | 土_偏印 template: "Your Earth carries the Alchemist's current…" | element page (journey sub-screen) = THE ONE depth home. **EP restructure (owner 2026-08-19): `k2_domain_readings` = the Ruling Domains god sub-blocks' Seeker layer (per present face: persona + domain chips + its seats + readings); `k2_overview`/`k2_functional` OFF-PAGE — kept as generation ore (the face switcher retired; sections 1–2 read ELEMENT_PAIR)** | **CORPUS LOCKED ×50 (owner 2026-08-19; 土_偏印 template + 49 batched persona-led, TG_PERSONA canonical incl. Virtuoso/Magistrate/Sage)** |
| `ELEMENT_PAIR` axis: `mechanism` (base + catalyst_turn/friction_turn) + `function` (primary + body + dips) | ¶ + turns · ¶ + lines | ELEMENT_PAIR ×25 (DM element × energy element; 20 relation pairs + 5 core self-pairs) | base 45–75w · turns ≤35w · body 40–70w · dips ≤20w | **ELEMENT-PAGE RESTRUCTURE (owner rulings ×4, 2026-08-19; MECHANISM + FUNCTION CONSOLIDATED 2026-09-01; FUNCTION-NOUN LAW TOTAL 2026-09-02):** section 1 = the hero (function claim + role-resolved `definition_catalyst`/`definition_friction` + equation + diagnosis; detail = mechanism base + turn + definition + dips) · section 2 = Ruling Domains. **Seat nouns and shadow nouns are RETIRED from all corpus and surfaces (REA_02 §5b/5b-ii): the mechanism bases no longer derive "your Drive/Root/Duty" — they conclude in the function claim; friction turns speak plain excess language; the definitions are role-variant continuations of the cta_verdict. v3 LOCKED ×50 (owner 2026-09-02: the flexible 3-¶ FUNCTION READING, 100–140w, card teaser = derived first sentence, own detail page + dips coda — app mapping live end to end).** **2026-09-03: THE AXIS HOMING (owner structure ruling): `definition_catalyst/friction` re-scoped to the SINGLE family-true define ¶ (≤55w, one define serves both 正/偏 siblings — the 金_木 merge is the proof) + NEW `advise_catalyst/advise_friction` (≤60w family-true role treatment); the reading assembles pair-define → god-ledger → pair-advise. CORPUS COMPLETE ×25 (owner 2026-09-04: migration ×5 + THE PAIR HARVEST ×20 — define/advise from the v3 ¶0/¶2 seed bank, all 40 ¶s gate-clean unedited; v3 texts retired to `__ore`; budgets define ≤55w/advise ≤60w enforced; the v3 fallback render removed from the app).** God-grain overviews/functionals leave the page, kept as generation ore. | 金_水 template: "Your core is Metal, and Metal feeds Water: dew beads on the cold blade…" | **WIRED (EP-D 2026-08-19): element page section 1 (capsule equation graphic + base + role-resolved turn) + section 2 (function chips w/ primary lit + body + dips); code home `src/content/pairs.js` (station-regenerated), seeder ELEMENT_PAIR section + _ORDER** | **CORPUS LOCKED ×25 + LIVE (owner 2026-08-19: template approved → batch → wired; §5f map locked with it; audits green). §5f re-ruled 2026-09-01 (Core→Body · Duty→Order, bijection): 5 self-pair function blocks re-authored Body-register, Duty primaries re-keyed, Drive dips re-keyed to Order (财生官), Voice bonds dips retired to `__ore`. NEW `cta_verdict` ×25 (owner formula 2026-09-01, LOCKED same day): ≤30w position-teaser-register line explaining "{Element} is your {Function}" on the dot card (graphic + definition + verdict)** |
| `ELEMENT_PAIR.carry` | {catalyst:{clause,remedy}, friction:{clause,remedy}, wide:{clause}} | ELEMENT_PAIR ×25 | clause ≤18w · remedy ≤12w | **THE CARRY CARD micro-lines (owner 2026-09-15, REA_02 §5h):** cut from the pair's locked mechanism turn (its first clause + its own directive); `wide` authored ×20 for the abundant catalyst (option 1); assembled on P4 in the manual's order | 金_土 friction: "The shelter closes over the blade: so much preparation that the edge never leaves the mine." → "Dig out and cut something real." | P4 carry card rows | **LIVE** |
| `POSITION.domain_readings` | per-domain ¶ | POSITION × declared domains (~172 ¶) | 35–60w each | **owner 2026-08-19: multi-domain positions expand into DEDICATED paragraphs per domain** (a Wealth reading ≠ a Love reading); renders in the seat panel after the summary reading, Seeker layer | 偏印在月支 → Mind ¶ + Growth ¶ + Career ¶ | seat accordions (Domains detail) | **CORPUS LOCKED ×70 (owner 2026-08-19, POS-D closed — REA_16 §6)** |
| `TG_PATTERN` axis: `line` + `reading` + `fused_line` (+ name/trigger metadata) | named-event cell ×9 | TG_PATTERN (god-pair canon, REA_02 §5g) | ≤40w · 70–100w · ≤25w | **owner 2026-08-19: the relation patterns are the 精华, a first-class CONDITIONAL reading angle** — triggered when both sides sit among the chart's resolved positions (`resolvePositions` detector), revealed in every participating seat's panel; FUSED tier (both sides one pillar) appends the fused line; one per seat, priority = canon order | 枭神夺食 fires on the exemplar's 偏印+食神 seats; 比劫夺财 FUSED on its hour pillar | seat accordions (CHART CHEMISTRY block, Seeker) | **CANON ×9 authored, awaiting owner lock** |
| **The distillation ladder** (derived surfaces ×3): catalogue takeaway strip (pattern line + deep-seat line, both deep-linked) · dot-card CTA seat subtext ("starts at {term}", minimal-card amendment 2026-09-01 — the body-line hint retired) · two-stage seat disclosure | derived one-liners + staging | DERIVED (SLOT_RANK ranking + firstSentence over locked corpus: TG_PATTERN.reading, POSITION.teaser) | first sentences | **DES_04 §AM.11 (owner rulings 2026-08-19, minimal dot card 2026-09-01): six-rung cognitive-load law — every level shows a derived distillation of the level below; ranking law REA_02 §5e; NO new authored content** | catalogue: "One law runs underneath your money: it attracts company." → taps into the Wealth seat | catalogue beat 1 · dot card CTA · seat accordions | LIVE (wired 2026-08-19, dot card slimmed 2026-09-01) |
| `palace_frames` ×7 | frame | POSITION | — | domain ≤14w + relational reframe | — | positional reads | RETIRED (superseded by the POSITION axis ×70) |
...
| `mean_line` ×5 | descriptor | ELEMENT (target ELEMENT·GOD) | ~15w | 1 sentence | Fire: The room is different before you speak — warmth as climate, light that changes what it touches. | K5 | RETIRED 2026-08-19 (with the MEAN table) |
...
| `tpl_relation_row` | {El} is your {Function} | Fire is your Order | E2 | LIVE |
...
| `tpl_carry_lead` ×3 | {Core} runs {Condition}. {nEase} energies feed a core already full. {nSeek} are where the surplus should go. (Underfueled mirrors; Balanced = the balanced note) | Metal runs Overfueled. Two energies feed a core already full. Three are where the surplus should go. | P4 carry card | LIVE (2026-09-15) |
| `ELEMENT_PAIR.carry.{missing,spared,thin,excess,unrooted}` | {clause, remedy} each | ×25 (+unrooted ×5 self cells) | as `carry` | **VALENCE × VOLUME state lines (owner 2026-09-16):** picked by buildCarryModel on (valence, volume): wanted+absent → missing · unwanted+absent → spared · unwanted+thin → thin · dominant → excess (the flip) · core absent → unrooted; each falls back to the pole's turn | 金_土 excess on 庚: "Too much earth clogs the metal: fed past use, the edge stalls, brooding where it should cut." | P4 carry card rows | **LIVE** |
| `ELEMENT_PAIR.carry_yin` | sparse {catalyst?:{clause?,remedy?}, friction?:{clause?,remedy?}, wide?:{clause?}} | ELEMENT_PAIR ×16 | as `carry` | **THE YIN SIBLING'S CARRY LINES (owner 2026-09-16):** overrides for the yin stem of the core element where the shared line names the yang archetype; code merges `carry_yin` over `carry` when `STEM_YIN[stem] === 1` (journeyData `buildCarryModel`) | 金_土 friction on 辛: "The setting closes over the stone: so much preparation that the jewel never leaves the box." → "Take it out and wear it somewhere real." | P4 carry card rows (yin stems) | **LIVE** |
| `STEM.gifts[].face` · `STEM.shadows[].face` · `echo_of` | face ∈ echo / wide / excess — **selection metadata only, never rendered (owner 2026-09-21): the selector reads it to pick a door's chip by volume; the reader never sees it. Not reading content, not reviewed as copy** (`dim` and `echo_of` are station-only authoring metadata) | ×140 | — | **THE DERIVATION LAW (owner R4, 2026-09-16; REA_16 §7):** item[0] per door = echo face cut from the pair definition; Body/Mind second = wide (gifts, from carry.wide) / excess (shadows, from carry.excess); selectPoolByDoor picks by the door's volume, one chip per open door, two or three per side | 庚 Mind: echo "Lives inside plans" ← 金_土.definition_friction · excess "Plans, never acts" ← 金_土.carry.excess | P4 gifts & shadows | **LIVE** |
...
From `archetypeSource.js` / `stemVariants.js` unless noted. **Rule: nothing here is authored further until ruled; it is source ore for the K2 pass.**
...
| `stemVariants.js` ×150 K1 variants | STEM·BAND·PATTERN | only `yourNature.desc` grain authored (庚 15) |
...
|---|---|---|---|
| A1–A6 | Reveal | `ui_labels`(V) · `archetype_name` · `manifesto`(couplet: L1 + identity-formula L2) · **stem-literacy line** (derived `庚 · YANG METAL`; replaced `stem_keywords` chips 2026-09-10) · `tpl_cast_line` · `stem_painting` | LOCKED (R1 ruled 2026-08-03; A4 re-ruled 08-05; chips retired 09-10) |
| B1–B4 | Hero | `archetype_name` · `manifesto` · **stem-literacy line** (derived `庚 · YANG METAL · CAST FROM YOUR DAY OF BIRTH`; replaced `stem_keywords` chips 2026-09-10) · `stem_seal` | LOCKED (B3 re-ruled 08-05; chips retired 09-10) |
...
| E1–E5 | Energy Manual + Map rails *(was Seek/Skip panels — retired 2026-09-10; SKIP→EASE)* | `ui_labels`(V) · condition defline (§5c) + why-lines · `tpl_relation_row` ← function noun (§5f) + presence · YOURSELF core tag · SEEK/EASE rail legend | LOCKED (journey restructure 09-10; REA_02 §5c amendment) |
...
| I1–I4 | Share card | `ui_labels`(V) · `archetype_name` · `manifesto` · **stem-literacy line** (derived; replaced `stem_keywords` chips 2026-09-10) · `tpl_share_coreline` · composites | LOCKED (chips retired 09-10) |
...
| R4 | `yourNature_desc` | **CORE CLOSED 2026-08-13 (BAND-A):** band variants ×30 authored under REA_16 (You-opener, band = how-it's-running) and locked; band resolution live on P4 via `resolveDayMasterReading`; the tone-stale 庚 ×15 pattern compounds retired (pattern grain retired — resolver falls band → baseline). **Riders RULED 2026-08-13 (owner):** J4 sub-screen deleted · `inscription` home = `dm_claims` claim 1 (REA_16 §6). |
...
| **Companions** | REA_02 (Dictionary — all V slots) · REA_06 (Ladder) · REA_01 (taxonomy) · REA_04 (knowledge sources) · REA_05 (generation pipelines) · REA_16 (voice + authoring prompt) · the archived v2.1 audit (v2.1 record) |
