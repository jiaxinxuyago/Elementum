...
> The *shape* of every field (type, tier, word caps, `varyBy` cardinality) is declared canonically in [`Elementum_App/src/contract/archetypeSchema.js`](../../Elementum_App/src/contract/archetypeSchema.js). The UI-surface companion is [REA_04 — Reading Generation Schema](REA_04_Reading_Generation_Schema.md).
> REA_06 tells you **how to write each field** (prompt templates, voice, classical grounding, quality gates). The schema tells you **what to write and how many variants are needed**.
