...
> - **Phrase law:** 2–3 everyday words naming what the person IS LIKE — an often-used, personality-first descriptor the target audience grasps at first glance ("Straight talker," "Own harshest critic"). NEVER "The + metaphor-noun" situation-titles ("The Held Line" is a situation, not a trait). No duplication of the stem's keywords or any other variable's key words. Middle vocabulary register.
> - **Angle Fork law (dimension law v2 — owner-ruled 2026-08-13, supersedes arena dimensions):** an arena (work · bonds · self …) is a STAGE, not an angle — the same trait restaged in four venues is one angle four times (the diagnosed failure: 癸's absorb/blurred/carries/depleted quartet all captioned "weak boundary"). The angle is the underlying TRAIT:
>   1. **Three-word reduction test (the law):** strip each item to its trait in ≤3 plain words. The pool's reductions must name different felt experiences — if two reductions could caption the SAME MOMENT of the user's life, they are one angle; refork. The reduction IS the item's `dim` value in the station (mechanically unique per pool — voice-audit enforces dim + phrase uniqueness within pools and phrase uniqueness across archetypes).
>   2. **One-trait ceiling:** the stem's headline cost/gift owns at most TWO items per pool; the rest mine secondary traits — every nature has them.
>   3. **Fork tools** (for when every arena restates the same quality — fork the FACULTY, not the venue): *miscalibration fork* (the gift when it's wrong, or doubted when right — 癸 "Feelings as facts" / "Doubts the signal") · *tolerance fork* (what the nature can't stand — 丙 "Allergic to gloom") · *counter-trait fork* (the quiet secondary trait — 甲's pride, 辛's thin skin, 己's preference-loss) · *tempo fork* (the trait across time — rumination, waiting, drift) · *reception fork* (how others misread it).
>   4. **Band-contradiction check:** opposite-calibration items (overtrust vs undertrust of one faculty) sit in opposite bands so no trio contains both.
