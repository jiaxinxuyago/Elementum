...
| HK-10 | **Customer-data backup task — ⏸ DEFERRED 2026-07-21 (owner call: "keep it hibernate and we'll use it until we have actual users onboard").** The scheduled task `Elementum Customer Data Backup` has been dead since 07-09 (fails `0x800710E0`); both destinations hold only `customer-data_2026-07-09.json`. Deliberately parked — Phase-0 owner-only, no real users, zero data-loss exposure. **NOT a live crisis; do not escalate as CRITICAL.** **Revive trigger = first real users onboard / before beta:** flip the three live-task flags (`AllowStartIfOnBatteries` + `DontStopIfGoingOnBatteries` + `StartWhenAvailable`), run once by hand to back-fill, then correct `PM_01` §3.6 (the held doc fix, which omits the same flags). Backup script `tools/backup-customer-data.mjs` stays HANDS-OFF. |
