...
| **B3** | Persona names | **2026-06-10 owner ruling stands** (The Mirror · Rival · Muse · Edge · Horizon · Steward · General · Arbiter · Alchemist · Sage). Other name sets → **non-surfacing aliases**. |
...
| **C4** | tgPattern cardinality | **DOC2/DOC9 5-pattern canonical**; Bible's 7-value scheme **superseded**. |
| **C5** | DOC4 §9 doctrine | **Exception carved** — energy-card content **does** vary by presence frame. |

---

## 3 · Per-document edit list (to execute on go-ahead)

### Reading schema → bump to v2.1 (`DOC6_Reading_Schema.md`)
...
- **DOC1**: document per-element resolution output + the split; **recompute the 庚 reference chart** with the polarity-aware resolver before it seeds any authoring.
- **DOC8**: retarget migration to `ENERGY_CARD_DATA` ×50 with presence-frame registers.
- **DOC10**: ensure the LLM payload carries the per-element god set.

### Archetype / knowledge
- **DOC2**: reconcile the 50-indexing to **DM-relative navigation**; disambiguate stem-archetypes (10 stems) vs TG-personas (10 gods); promote the persona layer from "planned."
- **DOC3**: add **presence-frame register** authoring to the 10 TG profiles; extend §8.3's 2-band model to 3-band; keep §8.6 as the no-jargon translation contract.
- **DOC9**: re-scope `dominantEnergy.characterDesc` from element-scoped to **persona-scoped**; add the **`rulingDomain`** axis (×50, DM-relative), distinct from the Western `lifeDomain` buckets.

### Generation
- **DOC4**: retarget Pipeline A2 → `ENERGY_CARD_DATA[element_god]` ×50 with presence-frame registers (dominant+absent bespoke, present derived) + `rulingDomain`; **carve the §9 exception** (C5).
- **DOC7**: add authoring sections — per-face persona prompt · register-variation prompt · ruling-domain prompt.
...
- **`DESIGN_AUDIT_BACKLOG.md`**: log v2.1 as **D14, extending D13**.

### Dispositions (C1–C4) — annotations (PDFs can't be edited in place; add index/sidecar notes)
- `Reading Schema.pdf` → "SUPERSEDED by DOC6_Reading_Schema (v2.1)."
...
- `BaZi_Analysis_Bible` 7-pattern → "Superseded by DOC2/DOC9 5-pattern."
...
- **⏳ POST-REVIEW + POST-ENGINE: REVISIT DOC6 Reading Schema — finalize the reading content design.** The schema (`DOC6_Reading_Schema.md`) is **provisional** until two things land: **(a)** the full doc review completes, and **(b)** the engine code fix runs (recomputing the 庚 reference chart's *real* faces, the per-element polarity splits, and confirming register + positional-data feasibility). Both surface concrete realities that will reshape the reading content design — the FACES content, the presence-frame registers, the `rulingDomain` lines, and the positional (宫位) readings. **Do a final DOC6 reconciliation pass then, before authoring any corpus.**
