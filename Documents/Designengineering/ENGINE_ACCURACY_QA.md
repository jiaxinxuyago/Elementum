...
**Findings:**
- **F1 (cleanup, not a bug):** `chart.tenGods.*Branch` = "—" (passes a branch to `getTenGod`, which only takes stems). Vestigial/unused — not surfaced. Either remove the field or derive it from the main hidden stem.
...
**What shipped** (`calculator.js`, per DOC1 §3.7 / §3.7b / §3.8):
