...
>
> Still open within §4.2: Seeker/Advisor **subscriptions** (their cards are disabled "Available after beta"), and tax/MoR. **Self-Report SHIPPED wired 2026-07-07**: real $6.99 Payment Link, account-gated buy, webhook routes products by amount_total (900=Founding, 699=Self-Report — ⚠ price changes must ship with a matching PRODUCTS update in workers/stripe-webhook), has_self_report is server-truth (RLS read, webhook write; the localStorage demo flip no longer grants). URL single-sourced in `infra/links.js`; display price in `infra/pricing.js`.
