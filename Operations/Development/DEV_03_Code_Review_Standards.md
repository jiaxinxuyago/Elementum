...
- **C5 — Contract fidelity.** Fields read by consumers exist in `contract/archetypeSchema.js`;
  content realizes what the schema declares (the REA_03 cascade). A consumer reading a
...
- **A10 — Change-set scope discipline.** A change-set (commit or small commit train)
  ...
  - **Coupled artifacts move together** — the same-change-set pairs are mandatory:
    price ⇄ webhook PRODUCTS map (S3) · schema ⇄ content/REA_03 (K3) · lazy() list ⇄
...
- **K3 — Doc cascade.** A change to `archetypeSchema.js` without the matching
  archetypeSource/REA_03 update (or an explicit note deferring it): MEDIUM. Engine
  methodology changes must not contradict DEV_01/REA_04 without doc updates in the same
  change-set: MEDIUM.
