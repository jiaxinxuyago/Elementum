...
| REA_17 | Generation_Prompt_Pack | The overview of the prompt database `Reading/Database/Prompts/` (v0.4, 2026-09-21; the prompt text moved there as one file per field, prompts as data): the rules of the game, the map of the master prompt, the input pack and the field cards, the comparison protocol pointer. Born 2026-09-21 reverse-engineered from the shipping corpus (v0.1–v0.3 carried the text inline): the master prompt (voice + audience + vocabulary + syntax laws in one block) · the input pack per axis · one field card per authored variable (surface, register, budget, construct, reasoning chain, checks, exemplar) · the like-for-like comparison protocol. Compiles REA_01–06 + REA_16; defines nothing new | *(new)* |
...
- `Reading/Database/Prompts/` — the prompt database (owner 2026-09-21: prompts are data, iterated like code): the master prompt, the input pack, the rules register, the comparison protocol and one card file per authored field; REA_17 is the overview that points here. `Reading/Database/Rewrites/<model>/` holds model rewrites per chart, never the station. `Reading/Database/templates/review-pack/` and `build-review-pack.mjs` were retired the same day (archive/README.md); the 2026-09-17 snapshot survives in `ComparativeAnalysis/Evaluations/elementum-review-pack/`.
- `ComparativeAnalysis/` — renamed from `Feedbacks/` 2026-09-21: `Prompts/` (the harness: chart reader, assembler, gate, dispatch, rewrites filer, read sheet, report) and `Evaluations/` (renamed from `Deliverables/`: the reports and rulings).
