...
| REA_17 | Generation_Prompt_Pack | The reading system's prompts, reverse-engineered from the shipping corpus (2026-09-21, DRAFT): the master prompt (voice + audience + vocabulary + syntax laws in one block) · the input pack per axis · one field card per authored variable (surface, register, budget, construct, reasoning chain, checks, exemplar) · the like-for-like comparison protocol. Compiles REA_01–06 + REA_16; defines nothing new | *(new)* |
