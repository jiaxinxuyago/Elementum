...
// ===================================================================
// Builds Reading/Database/templates/by_axis/json/<AXIS>/<archetype>.json
// — one folder per axis of the taxonomy (REA_01, normative), one
// placeholder template per archetype of that axis. Each file carries the
// archetype's identity + CANDIDATE variables harvested from the live
// corpus and the REA_02 locks — all pending the owner's per-axis
// construct ruling. Also emits _ORDER.json (the canonical archetype
// order per axis) which build-template-twins.mjs uses to order the
// generated by_variable pivot view.
...
// _ORDER.json — same drift law as the templates themselves.
const orderFp = path.join(OUT, '_ORDER.json');
const orderNext = JSON.stringify(order, null, 2) + '\n';
if (CHECK) {
  if (!fs.existsSync(orderFp)) { console.log('MISSING  _ORDER.json'); drift++; }
  else if (fs.readFileSync(orderFp, 'utf8') !== orderNext) { console.log('DRIFT    _ORDER.json'); drift++; }
