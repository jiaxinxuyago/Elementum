...
BANNED VOCABULARY — any occurrence in output text fails (REA_04 §8.9):
...
// ─── TRANSLATION PROTOCOL — REA_04 §8.9 Comprehensive Forbidden Term List ───────
// Any match in user-facing output text fails the quality gate. Keep in sync with
// REA_04 PART 8 §8.9. Internal prompt labels (Chinese refs, TG_MECHANICS names) are
...
// Detect any Chinese/CJK characters in generated output — hard fail per REA_04 §8.1
...
function qualityCheckPersona(key, p) {
  ...
  // Translation protocol gate — REA_04 §8.9
  ...
  if (hasChinese(allPersonaText)) issues.push("Chinese characters found in persona output — forbidden in user-facing text (REA_04 §8.1)");
  return issues;
}
...
function qualityCheckReading(key, t) {
  ...
  if (hasChinese(allText)) issues.push("Chinese characters found in reading output — forbidden in user-facing text (REA_04 §8.1)");
  return issues;
}
...
function qualityCheckAngle(key, a) {
  ...
  FORBIDDEN.forEach(term => { if (allText.toLowerCase().includes(term.toLowerCase())) issues.push(`Forbidden: "${term}"`); });
  if (hasChinese(allText)) issues.push("Chinese characters found in angle output — forbidden in user-facing text (REA_04 §8.1)");
  return issues;
}
...
function qualityCheckCompound(key, card) {
  ...
  // Translation protocol gate — REA_04 §8.9 (was missing from compound checker)
  ...
  if (hasChinese(allCompoundText)) issues.push("Chinese characters found in compound card — forbidden in user-facing text (REA_04 §8.1)");
  return issues;
}
